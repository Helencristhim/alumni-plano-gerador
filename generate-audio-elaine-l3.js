...
const BASE = '/Users/helenmendes/alumni-plano-gerador/public';

// Full audioMap from elaine-mieko-pinho.html lines 10-158
const audioMap = {
  ...
  // === LESSON 3 entries (lines 113-158) ===
...
function chooseVoice(text) {
  ...
  if (words.length <= 2) return ARTHUR;
  // Male dialogue context (Mark speaking, captain, male roles)
  const lower = text.toLowerCase();
  if (lower.includes('i am mark') || lower.includes('my name is mark') || lower.includes('captain speaking') || lower.includes('welcome aboard. i am mark')) return ARTHUR;
  // Elaine speaking or female context
  if (lower.includes('elaine asks') || lower.includes('elaine says') || lower.includes('my name is elaine') || lower.includes('i am elaine') || lower.includes('ms. pinho') || lower.includes('i am sarah') || lower.includes('i am from brazil')) return ELLEN;
  // 3+ words: alternate Arthur/Ellen
  const voice = alternateIndex % 2 === 0 ? ARTHUR : ELLEN;
  alternateIndex++;
  return voice;
}
...
async function main() {
  ...
  for (const [text, filePath] of entries) {
    ...
    toGenerate.push({ text, filePath, fullPath });
  }

  console.log(`\nTotal audioMap entries: ${entries.length}`);
  console.log(`Already exist (>1KB): ${skipped}`);
  console.log(`Need generation: ${toGenerate.length}\n`);

  if (toGenerate.length === 0) {
    console.log('All audio files already exist. Nothing to do.');
    return;
  }

  for (let i = 0; i < toGenerate.length; i++) {
    const { text, filePath, fullPath } = toGenerate[i];
    const voice = chooseVoice(text);
    const voiceName = voice === ARTHUR ? 'Arthur' : 'Ellen';
    console.log(`[${i + 1}/${toGenerate.length}] ${voiceName} | "${text.substring(0, 60)}${text.length > 60 ? '...' : ''}" -> ${path.basename(filePath)}`);
    ...
    if (i < toGenerate.length - 1) await sleep(500);
  }
