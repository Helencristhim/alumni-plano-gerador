#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 14 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp14, (3) ex-lesson-14 accordion,
(4) complementary aula 14 block, (5) audioMap entries; bumps totalLessons 13->14.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-14"' in html and 'id="stamp14"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 14 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula13 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula13.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp13 div (within stamps-row)
    sm = 'id="stamp13"'
    si = html.index(sm)
    ...
    # 3. ACCORDION -- insert after ex-lesson-12 closes, i.e. right before '</div><!-- /tab-exercises -->'
    ...
    # 4. COMPLEMENTARY -- insert the aula-13 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=13;', 'var totalLessons=14;')
    if html2 != html:
