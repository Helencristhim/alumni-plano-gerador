#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 14 (Reading Contracts) nos hubs prof + aluno de
...
Aulas 1-13 ficam intactas. Prof recebe card IN CLASS + stamp + accordion +
...
def patch(path, is_prof):
    ...
    if 'id="ex-lesson-14"' in s or 'data-media="l14-' in s:
        print(f'  SKIP {os.path.basename(path)} (aula 14 ja integrada)')
        return
    orig = len(s)

    # 1. CARD IN CLASS (só prof) — após o </a> da aula 13
    if is_prof:
        anchor = '/professor/walyson-ginaldo-silva-aula13.html?autostart=1'
        i = s.index(anchor)
        close = s.index('</a>', i) + len('</a>')
        s = s[:close] + '\n' + card + s[close:]

    # 2. STAMP — após o stamp13
    m = re.search(r'<div class="stamp" id="stamp13"[^>]*></div>', s)
    assert m, 'stamp13 nao encontrado em ' + path
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 3. ACCORDION Pre-class — após o lesson-card ex-lesson-13 (div balanceada)
    j = s.index('id="ex-lesson-13"')
    card_start = s.rindex('<div', 0, j)
    end = div_end(s, card_start)
    s = s[:end] + '\n' + accordion + s[end:]

    # 3b. COMPLEMENTARES — após o wrapper l13-youtube (div balanceada)
    k = s.index('data-media="l13-youtube"')
    ...
    s, n = re.subn(r'totalLessons=13\b', 'totalLessons=14', s)
    assert n >= 1, 'totalLessons=13 nao encontrado em ' + path
