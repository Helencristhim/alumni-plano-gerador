#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build standalone Aula 19 (Elaine) -- Small Talk: Meeting People. Splices authored
content into the proven aula9 scaffold (CSS+JS verbatim). Mirrors build_aula18.py.
...
a aula ENTREGUE e a INLINE no monolitico (ver inline_19.py)."""
import os, re, json
...
PROF_OUT  = os.path.join(REFDIR, 'elaine-mieko-pinho-aula19.html')
ALUNO_OUT = os.path.join(REFDIR, 'elaine-mieko-pinho-aula19-aluno.html')
...
def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 19 | Small Talk: Meeting People')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 19</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Small Talk -- Meeting People</div><div style="font-size:.8rem;color:var(--text-dim)">Starting a friendly conversation -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
...
print('OK aula19 standalone (ref) built; audioMap keys', len(phrases))
