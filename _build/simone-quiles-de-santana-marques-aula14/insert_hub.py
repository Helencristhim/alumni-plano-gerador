# -*- coding: utf-8 -*-
"""Insere os snippets da aula 14 nos hubs PROF e ALUNO da Simone (aditivo).
Adiciona o stamp14 (ainda nao existe no hub) e ajusta totalLessons para 14."""
import os, re
...
audio_lines += f'\n  "[order-l14]": "{AUDIO_BASE}a14_order_sequence.mp3",'

# menu card (snippet section 1)
mc = re.search(r'1\. CARD.*?-->\n(.*?)\n\n<!-- 2\.', snip, re.S)
menu_card = mc.group(1).rstrip('\n')

# stamp14 (snippet section 2)
ms = re.search(r'2\. STAMP.*?-->\n(.*?)\n\n<!-- 3\.', snip, re.S)
stamp14 = ms.group(1).rstrip('\n')
assert 'id="stamp14"' in stamp14, 'stamp14 ausente no snippet (config tem stamp id 14?)'

def patch(path, is_prof):
    s = open(path, encoding='utf-8').read()
    orig_len = len(s)
    assert 'id="ex-lesson-14"' not in s, f'{path}: aula 14 ja inserida'
    assert 'id="stamp14"' not in s, f'{path}: stamp14 ja existe'
    assert 'id="ex-lesson-13"' in s, f'{path}: aula 13 ausente (hub desatualizado?)'
    ...
    # 5) stamp14 na stamps-row (apos stamp13)
    m13 = re.search(r'(<div class="stamp" id="stamp13"[^>]*></div>)', s)
    assert m13, f'{path}: stamp13 ausente na stamps-row'
    s = s.replace(m13.group(1), m13.group(1) + '\n' + stamp14, 1)

    # 6) totalLessons -> 14 (NAO mexer em TOTAL_AULAS=48, que e o total do programa)
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 14', s)
