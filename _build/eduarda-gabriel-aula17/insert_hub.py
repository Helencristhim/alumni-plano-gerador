#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-17 ja existe, aborta.
Molde: eduarda-gabriel-aula16/insert_hub.py. Ancora de complementares = l16-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l17]": "/audio/eduarda-gabriel/order_l17_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-16">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l17 apos o ultimo wrapper l16 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula16.html?autostart=1')
