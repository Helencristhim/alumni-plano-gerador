...
USO
    python3 _build/joice-lopes-leite-extras/patch_extras_progress.py --hub public/aluno/joice-lopes-leite.html [--dry-run]

COPIA do _build/model/patch_extras_progress.py (que tem as abas do Diego escritas
no JS). Mudam so a lista ABAS e os seletores do clique; o resto e identico, para
o bloco publicado no hub do Diego nao derivar.
"""
import argparse
import re
import sys
...
BLOCO = ABRE + '''
<script>
/* Barrinha de % das abas suplementares da Joice (Musicas / Expressoes /
   Seu trabalho / TEDx). Mesma conta do Pre-class, em atributos proprios `data-extra-*`.
   Ver _build/model/patch_extras_progress.py. */
(function () {
  var ABAS = [
    { tab: 'music',       cards: '.lesson-card[id^="mu-song-"]' },
    { tab: 'expressions', cards: '.lesson-card[id^="ev-group-"]' },
    { tab: 'field',       cards: '.lesson-card[id^="fd-block-"]' },
    { tab: 'stage',       cards: '.lesson-card[id^="tx-step-"]' }
  ];
  ...
  document.addEventListener('click', function (ev) {
    var alvo = ev.target && ev.target.closest ? ev.target.closest('#tab-music, #tab-expressions, #tab-field, #tab-stage') : null;
