#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Gera os MP3 ElevenLabs das 4 abas suplementares da Joice e alimenta os DOIS audioMaps.
...
* ADITIVO: os MP3 tem prefixo proprio (mu/ev/fd/tx), que nao existe no material
  atual (que usa pc_*/a1_*..a20_*), entao nenhum arquivo existente e sobrescrito. No
...
SLUG = 'joice-lopes-leite'
# OS DOIS hubs. O material do professor e o do aluno carregam audioMaps IDENTICOS
# (333 entradas hoje), e as abas novas entram nos dois -- se o audio entrasse so
# num deles, o botao Listen ficaria mudo do outro lado, calado, sem erro.
HUBS = [os.path.join(RAIZ, 'public', 'aluno', SLUG + '.html'),
        os.path.join(RAIZ, 'public', 'professor', SLUG + '.html')]
...
SNIPPETS = [('mu', 'musica.html', ('data-phrase',), ('arthur', 'ellen')),
            ('ev', 'expressions.html', ('data-phrase', 'data-speak'), ('arthur', 'ellen')),
            ('fd', 'field.html', ('data-phrase', 'data-speak'), ('arthur', 'ellen')),
            ('tx', 'tedx.html', ('data-phrase',), ('arthur', 'ellen'))]
...
def frases_dos_snippets():
    ...
    for prefixo, arquivo, atributos, vozes in SNIPPETS:
        ...
        # VOZ DECLARADA NO CARD MANDA. O speech card do material dela nasce com
        # data-voice="ellen" porque a frase e da ALUNA: o modelo que ela imita
        # tem de ser uma voz feminina, nao a alternancia global. Onde nao ha
        # data-voice (botao Listen do vocab card), vale a REGRA 7 de sempre.
        forcada = {}
        for m in re.finditer(r'class="speech-card" data-phrase="([^"]+)" data-voice="([a-z_0-9]+)"', html):
            forcada[m.group(1)] = m.group(2)
        ...
        for m in re.finditer(padrao, html):
            ...
            if txt in vistas:
                continue
            vistas.add(txt)
            voz_do_card = forcada.get(m.group(1))
            achadas.append((prefixo, txt, (voz_do_card, voz_do_card) if voz_do_card else vozes))
    return achadas
...
def main():
    ...
    for hub in HUBS:
        print('-- %s' % os.path.relpath(hub, RAIZ))
        injeta_audiomap(plano, hub)
        limpa_orfaos(plano, hub)


def limpa_orfaos(plano, HUB=None):
    ...
    meus = re.compile(r'"/audio/%s/(mu|ev|fd|tx)_' % re.escape(SLUG))
    mantidas, removidas = [], []
    for ln in linhas:
    ...
    sumiram = [a for a in os.listdir(OUT)
               if a.startswith(('mu_', 'ev_', 'fd_', 'tx_')) and a not in usados]
    for a in sumiram:
...
def injeta_audiomap(plano, HUB=None):
    ...
    marca = '\n  // --- abas suplementares (Musicas / Expressoes / Seu trabalho / TEDx) ---\n'
    hub = hub[:fim] + marca + '\n'.join(novas) + hub[fim:]
    with open(HUB, 'w', encoding='utf-8') as f:
        f.write(hub)
