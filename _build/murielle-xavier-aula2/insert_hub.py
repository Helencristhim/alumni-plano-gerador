#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 2 no hub existente da Murielle (prof + aluno), de forma ADITIVA.
Stamp ja existe (stamp2 presente desde o hub 'new' da aula 1) — nao re-inserir.
Modelado em _build/danilo-febba-aula2/insert_hub.py."""
import os

ROOT = '/Users/helenmendes/alumni-plano-gerador'
SNIP = os.path.join(ROOT, '_build/murielle-xavier-aula2/hub_snippets.html')
N = 2
...
def process(path, href_prefix):
    ...
    orig = s
    this_card = card.replace('/professor/murielle-xavier-aula2.html', f'{href_prefix}/murielle-xavier-aula2.html')

    # 1. IN CLASS card: only the professor hub has a tab-inclass with aula cards.
    #    Aluno hub has no IN CLASS tab.
    anchor_a1 = f'<a href="{href_prefix}/murielle-xavier-aula1.html?autostart=1"'
    if anchor_a1 in s:
