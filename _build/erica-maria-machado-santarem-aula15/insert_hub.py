#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 15 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-14 intactas. Idempotente: se ex-lesson-15 ja existe, aborta.
Molde aula14: ancoras stamp14/ex-lesson-14/l14-youtube/aula14.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp15" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l15]": "/audio/erica-maria-machado-santarem/pc15_order_l15.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp15 apos stamp14 — SO se ainda nao existir
    if 'id="stamp15"' not in s:
        m = re.search(r'<div class="stamp" id="stamp14"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp15 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula14.html?autostart=1')
