#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 10 nos hubs prof e aluno de helio-santana.
Extrai as 5 seções do hub_snippets.html e as encaixa por âncora de string.
NÃO toca nas aulas 1-9 — só APPEND."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula9_link = '/professor/helio-santana-aula9.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula9_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula9 não encontrado ({kind})'
        a9_block = m.group(1)
        assert t.count(a9_block) == 1, f'card aula9 ambíguo ({kind})'
        t = t.replace(a9_block, a9_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp9
    stamp9 = '<div class="stamp" id="stamp9" data-label="Reportar al Equipo" style="background-image:url(\'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=200&q=80\')"></div>'
    assert stamp9 in t, f'âncora stamp9 não encontrada ({kind})'
    assert t.count(stamp9) == 1, f'stamp9 ambíguo ({kind})'
    t = t.replace(stamp9, stamp9 + '\n' + stamp, 1)

    # 3. ACCORDION -> antes do fechamento da tab-exercises (após o ex-lesson-9)
    ...
    # 6. totalLessons 9 -> 10 (TOTAL_AULAS fica 50)
    t2 = t.replace('var totalLessons=9', 'var totalLessons=10')
