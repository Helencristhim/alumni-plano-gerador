#!/usr/bin/env python3
"""Inserção ESTRITAMENTE ADITIVA da aula 20 nos hubs prof + aluno da Fabiana.
Âncoras de string inequívocas. Idempotente: aborta se a20 já existe."""
import os
...
def patch(path, is_prof):
    ...
    if 'ex-lesson-20' in html or 'stamp20' in html:
        print(f'  SKIP {os.path.basename(path)} — aula20 já presente'); return
    orig = html

    # STAMP: após o </div> do stamp19
    a = '<div class="stamp" id="stamp19"'
    ...
    # COMPLEMENTARES: após o wrapper l19-youtube
    pos = end_of_wrapper(html, html.index('data-media="l19-youtube"'))
    html = html[:pos] + '\n' + COMP + html[pos:]

    # MENU IN CLASS (só prof): após o </a> do card da aula 19
    if is_prof:
        m = html.index('</a>', html.index('aula19.html?autostart=1')) + len('</a>')
        html = html[:m] + '\n    ' + MENU.strip() + html[m:]
    ...
    # totalLessons 19 -> 20
    html = html.replace('var totalLessons = 19;', 'var totalLessons = 20;')

    assert html != orig and 'ex-lesson-20' in html, 'inserção falhou'
