#!/usr/bin/env python3
# Insere os snippets da aula 12 nos hubs prof + aluno do Victor (SO ADITIVO).
import re

SNIP = open('_build/victor-malvezi-paschotto-aula12/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/victor-malvezi-paschotto-aula11.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('victor-malvezi-paschotto-aula12.html?autostart') == 0, 'aula12 card ja existe'
    ...
    # --- 3. ACCORDION Pre-class (after ex-lesson-11) ---
