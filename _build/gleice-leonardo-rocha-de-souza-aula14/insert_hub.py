# -*- coding: utf-8 -*-
"""Insere os snippets da Aula 14 nos hubs PROF e ALUNO da Gleice (aditivo).
ADD ONLY -- nunca toca aulas 1-13. Adiciona stamp14, accordion Pre-class (ex-lesson-14),
bloco de Complementares (l14), card no menu IN CLASS (prof) -> standalone#slides,
merge das entradas a14_/pc14_ + [order-l14] no audioMap, e totalLessons -> 14."""
import os, re
...
# audioMap entries (snippet section 4) + [order-l14]
m = re.search(r'4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
audio_lines = m.group(1).rstrip('\n')
audio_lines += f'\n  "[order-l14]": "{AUDIO_BASE}a14_order_recommend.mp3",'
...
"onclick=\"location.href='/professor/gleice-leonardo-rocha-de-souza-aula14.html#slides'\" "
...
'    <div><div style="font-weight:600;font-size:.95rem">The Best Choice</div><div style="font-size:.8rem;color:var(--text-dim)">Why We Are the Best: Superlatives -- 27 slides</div></div>\n'
'  </div>\n')

CARD13_END = ('Comparatives -- 27 slides</div></div>\n'
              '  </div>\n')

STAMP14 = ("\n        <div class=\"stamp\" id=\"stamp14\" data-label=\"Best Choice\" "
           "style=\"background-image:url('https://images.unsplash.com/photo-1553877522-43269d4ea984?w=200&q=80')\"></div>")
...
def patch(path, is_prof):
    ...
    assert 'id="ex-lesson-14"' not in h, f'{path}: aula 14 ja inserida'
    assert 'id="stamp14"' not in h, f'{path}: stamp14 ja existe'
    ...
    # 3) bloco de Complementares (l14) antes do fim da #tab-complementary
    ...
    # 4) stamp14 na stamps-row (apos stamp13)
    m13 = re.search(r'<div class="stamp" id="stamp13"[^>]*></div>', h)
    assert m13, f'{path}: stamp13 ausente'
    h = h.replace(m13.group(0), m13.group(0) + STAMP14, 1)

    # 5) menu card IN CLASS (so prof)
    if is_prof:
        h = must(h, CARD13_END, CARD13_END + MENU_CARD)

    # 6) totalLessons -> 14
    h = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons=14', h)
