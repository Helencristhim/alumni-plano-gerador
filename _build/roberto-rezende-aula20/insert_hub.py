#!/usr/bin/env python3
# Insere os snippets da aula 20 nos hubs prof + aluno do roberto-rezende (SO ADITIVO).
# Legado: hub aluno tem buracos (ex-lesson 7-9, stamps 7-11 faltando), PRE-EXISTENTE.
# NAO consertamos: so anexamos a aula 20 apos o ULTIMO stamp/ex-lesson presente em CADA arquivo.
import re

SNIP = open('_build/roberto-rezende-aula20/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    # --- 1. MENU CARD (prof only) — apos o card da aula 19 ---
    if is_prof:
        anchor = '/professor/roberto-rezende-aula19.html?autostart=1'
        assert h.count('roberto-rezende-aula20.html?autostart') == 0, 'aula20 card ja existe'
        i = h.index(anchor)
    ...
    li = h.index('id="ex-lesson-19"')
    card_end = end_of_wrapper(h, li)
    h = h[:card_end] + '\n' + accordion.strip() + '\n' + h[card_end:]

    # --- 4. COMPLEMENTARY (apos o ULTIMO wrapper l19, dentro de tab-complementary) ---
    assert 'data-media="l20-' not in h, 'l20 complementary ja existe'
    last_l19 = [m.start() for m in re.finditer(r'data-media="l19-', h)][-1]
    wrap_end = end_of_wrapper(h, last_l19)
    h = h[:wrap_end] + '\n' + comp + '\n' + h[wrap_end:]
    ...
    # --- 6. totalLessons 19 -> 20 (ADITIVO) ---
    h2 = re.sub(r'var totalLessons ?= ?19', 'var totalLessons=20', h)
