#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insert builder-produced hub_snippets.html (aula 19) into Marlene's existing hubs.
ADDITIVE ONLY: never touches lessons 1-18. Idempotent (aborts if stamp19 already present)."""
import os
import re
...
def insert_common(c):
    assert 'id="stamp19"' not in c, 'stamp19 já presente — abortando (não duplicar)'
    # (b) stamp after stamp18
    m = re.search(r'(<div class="stamp" id="stamp18"[^>]*></div>)', c)
    c = c[:m.end()] + '\n        ' + STAMP + c[m.end():]
    # (c) ex-lesson-19 accordion right before the Celebration card
    anchor = '\n<!-- Celebration card -->'
    if anchor in c:
        i = c.index(anchor)
    else:
    ...
    c = c[:m.end()] + AUDIO + '\n' + c[m.end():]
    # totalLessons 18 -> 19
    c = re.sub(r'var totalLessons=18', 'var totalLessons=19', c)
    c = re.sub(r'var totalLessons = 18', 'var totalLessons = 19', c)
    return c
...
c = insert_common(c)
# (a) menu card: after the aula-18 menu <a>...</a> card.
mark = 'marlene-landucci-aula18.html?autostart=1'
mi = c.index(mark)
