#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 10 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-9 intactas. Idempotente: se ex-lesson-10 ja existe, aborta.
Molde aula9: ancoras stamp9/ex-lesson-9/l9-youtube/aula9.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp10" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l10]": "/audio/erica-maria-machado-santarem/pc10_order_l10.mp3",'
...
def patch(path, is_prof):
    ...
    if 'id="ex-lesson-10"' in s:
        print(f'  SKIP {os.path.basename(path)} (aula 10 ja integrada)')
        return
    orig_len = len(s)

    # 1. stamp10 apos stamp9 — SO se ainda nao existir
    if 'id="stamp10"' not in s:
        m = re.search(r'<div class="stamp" id="stamp9"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp10 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')

    # 2. accordion ex-lesson-10 apos ex-lesson-9
    st = s.index('<div class="lesson-card" id="ex-lesson-9">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l10 apos o ultimo wrapper l9
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula9.html?autostart=1')
        ce = s.index('</a>', ci) + len('</a>')
        s = s[:ce] + '\n' + card_prof + s[ce:]

    # 5. entradas de audioMap (logo apos a abertura do objeto) + [order-l10]
