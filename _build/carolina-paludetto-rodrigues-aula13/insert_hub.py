#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 13 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula12.

Aulas 1-12 intactas. Idempotente: se ex-lesson-13 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp13, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l13]) e ajusta totalLessons=13.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l13]": "/audio/carolina-paludetto-rodrigues/pc13_order_sequence.mp3",'
...
assert 'id="stamp13"' in stamp, stamp[:60]
assert 'id="ex-lesson-13"' in accordion, accordion[:60]
assert 'data-media="l13-youtube"' in comp, comp[:60]
assert amap_entries and 'a13_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp12 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]
    ...
    # 3. complementares l13 apos o ultimo wrapper de l12
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula12.html?autostart=1')
