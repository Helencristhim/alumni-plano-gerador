#!/usr/bin/env python3
# Insere os snippets da aula 14 nos hubs prof + aluno do roberto-rezende (SO ADITIVO).
# Legado: hub aluno tem buracos (ex-lesson 7-9, stamps 7-11 faltando), PRE-EXISTENTE.
# NAO consertamos: so anexamos a aula 14 apos o ULTIMO stamp/ex-lesson presente em CADA arquivo.
import re

SNIP = open('_build/roberto-rezende-aula14/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    # --- 1. MENU CARD (prof only) — apos o card da aula 13 ---
    if is_prof:
        anchor = '/professor/roberto-rezende-aula13.html?autostart=1'
        assert h.count('roberto-rezende-aula14.html?autostart') == 0, 'aula14 card ja existe'
        i = h.index(anchor)
    ...
    last_l13 = [m.start() for m in re.finditer(r'data-media="l13-', h)][-1]
    wrap_end = end_of_wrapper(h, last_l13)
    h = h[:wrap_end] + '\n' + comp + '\n' + h[wrap_end:]
    ...
    # --- 6. totalLessons 13 -> 14 (ADITIVO) ---
    h2 = re.sub(r'var totalLessons ?= ?13', 'var totalLessons=14', h)
