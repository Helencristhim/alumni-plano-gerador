#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insercao ADITIVA da aula 20 do Carlos nos hubs (prof + aluno).
So ADICIONA: stamp20, accordion Pre-class (ex-lesson-20), card IN CLASS (so prof),
bloco Complementares (l20), entradas de audioMap, e totalLessons 19->20.
NAO toca em nada das aulas 1-19. Cada ancora e validada (exatamente 1 ocorrencia)."""
import os, re, sys
...
STAMP20 = ('        <div class="stamp" id="stamp20" data-label="Sealing the Deal" '
           "style=\"background-image:url('https://images.unsplash.com/photo-1521791136064-7986c2920216?w=200&q=80')\"></div>")

INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula20.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 3 &#8212; Sealing the Deal: The Sign-Off and What Comes After</div><div style="font-size:.8rem;color:var(--text-dim)">The wording is final &#8212; now sign and look ahead. Execute and countersign the agreement, set the effective date, then plan what comes after with the future perfect &#8212; "By the close, the teams will have integrated" &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-20"' not in s, f'{path}: aula 20 ja integrada'
    # 1. stamp20 apos stamp19
    s = sub_once(s, STAMP19, STAMP19 + '\n' + STAMP20, 'stamp')
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=19;', '  var totalLessons=20;', 'totalLessons')
    # 6. audioMap entries
