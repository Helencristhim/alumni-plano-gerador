# -*- coding: utf-8 -*-
"""Insere os snippets da aula 16 nos hubs PROF e ALUNO da Simone (aditivo).
Adiciona o stamp16 (ainda nao existe no hub) e ajusta totalLessons para 16."""
import os, re
...
audio_lines += f'\n  "[order-l16]": "{AUDIO_BASE}a16_order_sequence.mp3",'

# menu card (snippet section 1)
mc = re.search(r'1\. CARD.*?-->\n(.*?)\n\n<!-- 2\.', snip, re.S)
menu_card = mc.group(1).rstrip('\n')

# stamp16 (snippet section 2)
ms = re.search(r'2\. STAMP.*?-->\n(.*?)\n\n<!-- 3\.', snip, re.S)
stamp16 = ms.group(1).rstrip('\n')
assert 'id="stamp16"' in stamp16, 'stamp16 ausente no snippet (config tem stamp id 16?)'

def patch(path, is_prof):
    s = open(path, encoding='utf-8').read()
    orig_len = len(s)
    assert 'id="ex-lesson-16"' not in s, f'{path}: aula 16 ja inserida'
    assert 'id="stamp16"' not in s, f'{path}: stamp16 ja existe'
    assert 'id="ex-lesson-15"' in s, f'{path}: aula 15 ausente (hub desatualizado?)'
    ...
    # 5) stamp16 na stamps-row (apos stamp15)
    m15 = re.search(r'(<div class="stamp" id="stamp15"[^>]*></div>)', s)
    assert m15, f'{path}: stamp15 ausente na stamps-row'
    s = s.replace(m15.group(1), m15.group(1) + '\n' + stamp16, 1)

    # 6) totalLessons -> 16 (NAO mexer em TOTAL_AULAS=48, que e o total do programa)
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 16', s)
