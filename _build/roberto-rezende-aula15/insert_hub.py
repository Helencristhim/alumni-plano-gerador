#!/usr/bin/env python3
# Insere os snippets da aula 15 nos hubs prof + aluno do roberto-rezende (SO ADITIVO).
# Legado: hub aluno tem buracos (ex-lesson 7-9, stamps 7-11 faltando), PRE-EXISTENTE.
# NAO consertamos: so anexamos a aula 15 apos o ULTIMO stamp/ex-lesson presente em CADA arquivo.
import re

SNIP = open('_build/roberto-rezende-aula15/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    # --- 1. MENU CARD (prof only) — apos o card da aula 14 ---
    if is_prof:
        anchor = '/professor/roberto-rezende-aula14.html?autostart=1'
        assert h.count('roberto-rezende-aula15.html?autostart') == 0, 'aula15 card ja existe'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        h = h[:close] + '\n            ' + menu_card.strip() + h[close:]

    # --- 2. STAMP (apos o ULTIMO stamp presente; difere prof vs aluno) ---
    assert 'id="stamp15"' not in h, 'stamp15 ja existe'
    last_stamp = [m.start() for m in re.finditer(r'id="stamp\d+"', h)][-1]
    sclose = h.index('</div>', last_stamp) + len('</div>')
    h = h[:sclose] + '\n        ' + stamp.strip() + h[sclose:]

    # --- 3. ACCORDION Pre-class (apos o FIM do card ex-lesson-14) ---
    assert 'id="ex-lesson-15"' not in h, 'ex-lesson-15 ja existe'
    li = h.index('id="ex-lesson-14"')
    card_end = end_of_wrapper(h, li)
    h = h[:card_end] + '\n' + accordion.strip() + '\n' + h[card_end:]

    # --- 4. COMPLEMENTARY (apos o ULTIMO wrapper l14, dentro de tab-complementary) ---
    assert 'data-media="l15-' not in h, 'l15 complementary ja existe'
    last_l14 = [m.start() for m in re.finditer(r'data-media="l14-', h)][-1]
    wrap_end = end_of_wrapper(h, last_l14)
    h = h[:wrap_end] + '\n' + comp + '\n' + h[wrap_end:]
    ...
    # --- 6. totalLessons 14 -> 15 (ADITIVO) ---
    h2 = re.sub(r'var totalLessons ?= ?14', 'var totalLessons=15', h)
