#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 18 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp18, accordion Pre-class (ex-lesson-18), card IN CLASS (só prof),
bloco Complementares (l18), entradas de audioMap, e totalLessons 17->18.
NÃO toca em nada das aulas 1-17 (inclusive os fixes da Helen). Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
STAMP18 = ('        <div class="stamp" id="stamp18" data-label="The Signing" '
           "style=\"background-image:url('https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=200&q=80')\"></div>")

INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula18.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 3 &#8212; The Signing: Reading the Agreement Before You Sign</div><div style="font-size:.8rem;color:var(--text-dim)">The deal is agreed and the agreement is drafted &#8212; now read it before you sign. Learn the language of a contract (clause, provision, liability, binding) and use defining relative clauses to pinpoint exactly which part does what &#8212; "the clause that limits liability" &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-18"' not in s, f'{path}: aula 18 já integrada'
    # 1. stamp18 após stamp17
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=17;', '  var totalLessons=18;', 'totalLessons')
    # 6. audioMap entries
