#!/usr/bin/env python3
"""Insere a aula 16 nos hubs prof e aluno da Karina -- SO ADITIVO.
...
def patch_hub(path, is_prof):
    s = read(path)
    n0 = len(s)
    assert 'ex-lesson-16' not in s, 'aula 16 JA esta no hub -- abortando (idempotencia)'

    # 1. STAMP -- apos o stamp14
    stamp15 = '<div class="stamp" id="stamp15"'
    end15 = s.index('</div>', s.index(stamp15)) + len('</div>')
    s = s[:end15] + '\n' + stamp + s[end15:]

    # 2. ACCORDION Pre-class -- apos o fechamento do ex-lesson-14
    anchor15 = '<div class="lesson-card" id="ex-lesson-15">'
    p15 = s.index(anchor15)
    nxt = s.find('<div class="lesson-card"', p15 + len(anchor15))
    tab_end = s.find('</div><!-- /tab-exercises -->', p15)
    ...
    yt = '<div class="media-card-wrapper" data-media="l15-youtube">'
    pyt = s.index(yt)
    ...
    if comp_tab_end != -1:
        ...
        assert m, 'nao achei fim do wrapper l15-youtube'
        end = pyt + m.end()
        s = s[:end] + '\n' + comp + s[end:]
    ...
    assert 'var totalLessons=15' in s, 'nao achei var totalLessons=15'
    s = s.replace('var totalLessons=15', 'var totalLessons=16')

    # 6 (so prof). IN CLASS card -- apos o card da aula 14
    if is_prof:
        card_anchor = '/professor/karina-macedo-aula15.html?autostart=1'
        pc = s.index(card_anchor)
        a_end = s.index('</a>', pc) + len('</a>')
        s = s[:a_end] + '\n' + card + s[a_end:]

    assert 'ex-lesson-16' in s and 'stamp16' in s and 'l16-series' in s, 'pos-condicao falhou'
    assert 'var totalLessons=16' in s
    if is_prof:
        assert 'karina-macedo-aula16.html?autostart=1' in s
    write(path, s)
