...
SNIP = open('_build/maria-claudia-curimbaba-aula18/hub_snippets.html', encoding='utf-8').read()
...
def process(path, is_prof):
    ...
    if is_prof:
        anchor = '<a href="/professor/maria-claudia-curimbaba-aula17.html?autostart=1"'
        card = card_raw.replace('aula18.html?autostart=1" ', 'aula18.html?autostart=1" target="_blank" ')
        assert anchor in html, f'{path}: IN CLASS aula17 anchor not found'
        html = insert_before(html, anchor, card.strip())

    # 2. stamp — before stamp17
    anchor = '<div class="stamp" id="stamp17"'
    assert anchor in html, f'{path}: stamp17 not found'
    html = insert_before(html, anchor, stamp_raw)

    # 3. accordion — before ex-lesson-17
    anchor = '<div class="lesson-card" id="ex-lesson-17">'
    assert anchor in html, f'{path}: ex-lesson-17 not found'
    html = insert_before(html, anchor, accord_raw)

    # 3b. complementary — before aula17 complementary header
    anchor = '<h4 style="font-family:\'Cormorant Garamond\',serif;font-size:1.1rem;color:var(--accent);margin-top:2rem;margin-bottom:.8rem">Aula 17 -- Data-Driven: Describing Trends and Numbers</h4>'
    assert anchor in html, f'{path}: complementary aula17 header not found'
    ...
    html, n = re.subn(r'totalLessons=17', 'totalLessons=18', html)
    assert n >= 1, f'{path}: totalLessons=17 not found'
