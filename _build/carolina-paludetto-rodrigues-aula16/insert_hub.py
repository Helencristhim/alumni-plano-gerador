#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 16 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula15.

Aulas 1-15 intactas. Idempotente: se ex-lesson-16 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp16, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l16]) e ajusta totalLessons=16.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l16]": "/audio/carolina-paludetto-rodrigues/pc16_order_sequence.mp3",'
...
assert 'id="stamp16"' in stamp, stamp[:60]
assert 'id="ex-lesson-16"' in accordion, accordion[:60]
assert 'data-media="l16-youtube"' in comp, comp[:60]
assert amap_entries and 'a16_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp15 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]

    # 2. accordion ex-lesson-16 apos ex-lesson-15
    st = s.index('<div class="lesson-card" id="ex-lesson-15">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l16 apos o ultimo wrapper de l15
    yi = s.index('data-media="l15-youtube"')
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula15.html?autostart=1')
        ce = s.index('</a>', ci) + len('</a>')
        s = s[:ce] + '\n' + card_prof + s[ce:]

    # 5. entradas de audioMap (logo apos a abertura do objeto) + [order-l16]
