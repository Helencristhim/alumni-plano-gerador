...
PROF_OUT  = os.path.join(ROOT, 'public', 'professor', 'elaine-mieko-pinho-aula15.html')
ALUNO_OUT = os.path.join(ROOT, 'public', 'aluno', 'elaine-mieko-pinho-aula15.html')
GEN_OUT   = os.path.join(ROOT, 'scripts', 'generate-elaine-aula15-audio.js')
...
AUDIT = '''<!--
SCENARIO FIT — Aula 15
Can-do: "I can shop for clothes: ask for sizes and colors, try things on, and check the price."
Gramatica-alvo: "Do you have + this in + size/color?"; "Can I try it on?"; "Is it on sale?"; "How much is it?" (revisao L11).
Vocab-alvo: size, color, try on, fitting room, cheap, expensive, discount, fit.
Cenario escolhido: uma loja de roupas em New York, pedindo tamanho/cor, experimentando e checando preco/desconto.
Por que elicita o alvo: comprar roupa OBRIGA o aluno a perguntar tamanho/cor, pedir para experimentar e checar o preco. >70% dos itens-alvo elicitados.

CONTINUIDADE — Aula 15
Itens novos: size, color, try on, fitting room, cheap, expensive, discount, fit; "Do you have...?", "Can I try it on?", "Is it on sale?".
Itens revisados (L11/L14): "How much is it?"; "Which subway...?", "How much is a ticket?".
Callback no warm-up (slide 2): retoma ATIVAMENTE 2 itens da Aula 14 — o aluno ouve e repete "Which subway goes to Times Square?" e "How much is a ticket?", e faz a ponte: pegou o metro ate as lojas, agora vai comprar roupa.
-->
'''

def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 15 | Shopping: Stores')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 15</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Shopping -- Stores</div><div style="font-size:.8rem;color:var(--text-dim)">Sizes, colors &amp; prices -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
