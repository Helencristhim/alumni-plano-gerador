# -*- coding: utf-8 -*-
"""Insere os snippets da aula 5 nos hubs PROF e ALUNO da Simone (aditivo).
NAO duplica o stamp5 (ja existe no hub). Ajusta totalLessons para 5."""
import os, re
...
# audioMap entries (from snippet section 4) + [order-l5]
snip = open(os.path.join(HERE, 'hub_snippets.html'), encoding='utf-8').read()
m = re.search(r'4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
audio_lines = m.group(1).rstrip('\n')
audio_lines += f'\n  "[order-l5]": "{AUDIO_BASE}a5_order_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    orig_len = len(s)
    assert 'id="ex-lesson-5"' not in s, f'{path}: aula 5 ja inserida'
    ...
    # 5) totalLessons -> 5
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 5', s)
