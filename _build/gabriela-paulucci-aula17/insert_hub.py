#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 da Gabriela nos hubs (prof + aluno) de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-17 ja existe, aborta.

Ambos os hubs (prof e aluno) ja tem a aula 16 como ultima aula (ex-lesson-16,
stamp16, l16). Por isso AMBOS ancoram em last_n=16."""
import os, re
...
def patch(path, is_prof, last_n):
    ...
    # 1. stamp17 apos o ultimo stamp existente
    m = re.search(r'<div class="stamp" id="stamp%d"[^>]*></div>' % last_n, s)
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 2. accordion ex-lesson-17 apos o ultimo ex-lesson existente
    st = s.index('<div class="lesson-card" id="ex-lesson-%d">' % last_n)
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l17 apos o ultimo wrapper l{last_n}
...
patch(os.path.join(ROOT, 'public', 'professor', 'gabriela-paulucci.html'), True, 16)
patch(os.path.join(ROOT, 'public', 'aluno', 'gabriela-paulucci.html'), False, 16)
print('OK')
