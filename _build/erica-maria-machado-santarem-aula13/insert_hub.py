#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 13 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-12 intactas. Idempotente: se ex-lesson-13 ja existe, aborta.
Ancoras: stamp12 / ex-lesson-12 / l12-youtube / aula12.html?autostart=1."""
import os, re
...
ORDER_AMAP = '  "[order-l13]": "/audio/erica-maria-machado-santarem/pc13_order_l13.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp13 apos stamp12 — SO se ainda nao existir
    if 'id="stamp13"' not in s:
        m = re.search(r'<div class="stamp" id="stamp12"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp13 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    # 3. complementares l13 apos o ultimo wrapper l12
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula12.html?autostart=1')
