#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 14 nos hubs prof e aluno de daniel-bastos.
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula13_link = '/professor/daniel-bastos-aula13.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula13_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula13 não encontrado ({kind})'
        a13_block = m.group(1)
        assert t.count(a13_block) == 1
        t = t.replace(a13_block, a13_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp13
    stamp13 = '<div class="stamp" id="stamp13" data-label="Datos y Gráficos" style="background-image:url(\'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=200&q=80\')"></div>'
    t = insert_after(t, stamp13, '\n' + stamp, f'{kind} stamp')
    ...
    # 6. totalLessons 13 -> 14 (TOTAL_AULAS fica 48)
    t2 = t.replace('var totalLessons=13;', 'var totalLessons=14;')
