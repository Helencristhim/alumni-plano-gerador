#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build standalone Aula 16 (Elaine) -- Review 2: Food and Getting Around. Splices authored
content into the proven aula9 scaffold (CSS+JS verbatim). Mirrors build_aula15.py."""
import os, re, json
...
PROF_OUT  = os.path.join(ROOT, 'public', 'professor', 'elaine-mieko-pinho-aula16.html')
ALUNO_OUT = os.path.join(ROOT, 'public', 'aluno', 'elaine-mieko-pinho-aula16.html')
GEN_OUT   = os.path.join(ROOT, 'scripts', 'generate-elaine-aula16-audio.js')
...
AUDIT = '''<!--
SCENARIO FIT -- Aula 16 (Review 2)
Can-do: "I can live a full day abroad: order food, mention allergies, pay, ask directions, take the subway, and shop."
Estruturas revisadas (Aulas 9-15): "I would like..."; "I am allergic to..."; "Could I have...?"; "Where is...? / Which...?"; "How much...? / Do you have...?".
Vocab revisado: menu, allergic, bill, order, directions, subway, ticket, discount (NENHUM vocab novo -- REGRA 22, whitelist de revisao).
Cenario escolhido: um dia completo em New York costurando os 7 momentos das aulas 9-15.
Por que elicita o alvo: uma simulacao de dia inteiro OBRIGA o uso de TODAS as estruturas em sequencia natural. >70% dos itens-alvo elicitados.

CONTINUIDADE -- Aula 16
Itens novos: NENHUM (aula de revisao/checkpoint).
Itens revisados: todos os das Aulas 9-15.
Callback no warm-up (slide 2): retoma ATIVAMENTE o repertorio do bloco (ordering > allergies > pay > directions > subway > shopping) antes da simulacao.
-->
'''

def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 16 | Review 2 -- Food and Getting Around')
    ...
    extra = ('\n<div class="stamp" id="stamp10" data-label="Allergies" style="background-image:url(\'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=200&q=80\')"></div>'
             ...
             '\n<div class="stamp" id="stamp15" data-label="Shopping" style="background-image:url(\'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=200&q=80\')"></div>'
             '\n<div class="stamp" id="stamp16" data-label="Review 2" style="background-image:url(\'https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?w=200&q=80\')"></div>')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 16</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Review 2 -- Food and Getting Around</div><div style="font-size:.8rem;color:var(--text-dim)">Full day simulation -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
...
print('OK aula16 standalone built; audioMap keys', len(phrases))
