#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 18 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-17 intactas. Idempotente: se ex-lesson-18 ja existe, aborta.
Ancoras: stamp17/ex-lesson-17/l17-youtube/aula17.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp18" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l18]": "/audio/erica-maria-machado-santarem/pc18_order_l18.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp18 apos stamp17 — SO se ainda nao existir
    if 'id="stamp18"' not in s:
        m = re.search(r'<div class="stamp" id="stamp17"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp18 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula17.html?autostart=1')
