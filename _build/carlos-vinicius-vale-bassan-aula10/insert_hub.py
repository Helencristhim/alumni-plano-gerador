#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 10 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp10, accordion Pre-class (ex-lesson-10), card IN CLASS (só prof),
bloco Complementares (l10), entradas de audioMap, e totalLessons 9->10.
NÃO toca em nada das aulas 1-9. Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula10.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 2 &#8212; Deal Structure &amp; Synergies</div><div style="font-size:.8rem;color:var(--text-dim)">Structuring the deal and presenting it to the board &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-10"' not in s, f'{path}: aula 10 já integrada'
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=9;', '  var totalLessons=10;', 'totalLessons')
    # 6. audioMap entries
