#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 12 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp12, accordion Pre-class (ex-lesson-12), card IN CLASS (só prof),
bloco Complementares (l12), entradas de audioMap, e totalLessons 11->12.
NÃO toca em nada das aulas 1-11. Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula12.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 3 &#8212; Leading Change &amp; the Change Team</div><div style="font-size:.8rem;color:var(--text-dim)">Lead people through the change: what the team must, has to, should, and needs to do &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-12"' not in s, f'{path}: aula 12 já integrada'
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=11;', '  var totalLessons=12;', 'totalLessons')
    # 6. audioMap entries
