...
amap_lines.append(f'  "[order-l8]": "{AUDIO_BASE}pc8_order_sequence.mp3",')
amap_block = '\n'.join(amap_lines)

# --- card IN CLASS (so prof), no formato das cards existentes da Anna ---
CARD8 = (
    '  <a href="/professor/anna-flavia-miranda-da-silva-aula8.html?autostart=1" target="_blank" '
    ...
    'display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem">08</div>\n'
...
STAMP7 = ('<div class="stamp" id="stamp7" data-label="Phone" style="background-image:url(\''
          'https://images.unsplash.com/photo-1423666639041-f56000c27a9a?w=200&q=80\')"></div>')
STAMP8 = ('\n        <div class="stamp" id="stamp8" data-label="Plans" style="background-image:url(\''
...
def patch(path, is_prof):
    ...
    s = s.replace(STAMP7, STAMP7 + STAMP8, 1); n += 1
    ...
    # 5. totalLessons
    assert 'var totalLessons=7;' in s, f'{path}: totalLessons=7 nao encontrado'
    s = s.replace('var totalLessons=7;', 'var totalLessons=8;', 1); n += 1

    # 6. card IN CLASS (so prof)
    if is_prof:
        assert s.count(CARD07_TAIL) == 1, f'{path}: ancora card 07 nao unica'
        s = s.replace(CARD07_TAIL, CARD07_TAIL + '\n' + CARD8, 1); n += 1
