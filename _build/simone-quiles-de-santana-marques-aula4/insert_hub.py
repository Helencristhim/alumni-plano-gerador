# -*- coding: utf-8 -*-
"""Insere os snippets da aula 4 nos hubs PROF e ALUNO da Simone (aditivo).
NAO duplica o stamp4 (ja existe no hub). Ajusta totalLessons para 4."""
import os, re
...
# audioMap entries (from snippet section 4) + [order-l4]
snip = open(os.path.join(HERE, 'hub_snippets.html'), encoding='utf-8').read()
m = re.search(r'4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
audio_lines = m.group(1).rstrip('\n')
audio_lines += f'\n  "[order-l4]": "{AUDIO_BASE}a4_order_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    orig_len = len(s)
    assert 'id="ex-lesson-4"' not in s, f'{path}: aula 4 ja inserida'
    ...
    # 5) totalLessons -> 4
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 4', s)
