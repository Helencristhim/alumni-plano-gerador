...
NAO toca aulas 1-16. So adiciona: card IN CLASS (so prof), stamp17, accordion
Pre-class, bloco de Complementares, entradas de audioMap (+ [order-l17]) e
ajusta var totalLessons 16->17. Fragmentos vem dos arquivos do builder.
"""
import os
import re
...
amap_lines.append(f'  "[order-l17]": "{AUDIO_BASE}pc17_order_l17.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp17 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp17".*?</div>)', snip, re.S)
stamp17 = ms.group(1)

STAMP16 = ('<div class="stamp" id="stamp16" data-label="Follow-Up" '
           "style=\"background-image:url('https://images.unsplash.com/"
           "photo-1450101499163-c8848c66ca85?w=200&q=80')\"></div>")

AULA16_HREF = 'percival-jr-aula16.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 2. stamp17 (apos stamp16 Follow-Up)
    ...
    assert len(re.findall(r'totalLessons=16(?![0-9])', s)) == 1, f'{path}: totalLessons=16 nao unico'
    s = re.sub(r'totalLessons=16(?![0-9])', 'totalLessons=17', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 16
    if is_prof:
        i = s.index(AULA16_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
