#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insert builder-produced hub_snippets.html (aula 21) into Marlene's existing hubs.
ADDITIVE ONLY: never touches lessons 1-20. Idempotent (aborts if stamp21 already present)."""
import os
import re
...
def insert_common(c):
    assert 'id="stamp21"' not in c, 'stamp21 já presente — abortando (não duplicar)'
    # (b) stamp after stamp20
    m = re.search(r'(<div class="stamp" id="stamp20"[^>]*></div>)', c)
    c = c[:m.end()] + '\n        ' + STAMP + c[m.end():]
    # (c) ex-lesson-21 accordion right before the Celebration card
    anchor = '\n<!-- Celebration card -->'
    if anchor in c:
        i = c.index(anchor)
    else:
    ...
    c = c[:m.end()] + AUDIO + '\n' + c[m.end():]
    # totalLessons 20 -> 21
    c = re.sub(r'var totalLessons=20', 'var totalLessons=21', c)
    c = re.sub(r'var totalLessons = 20', 'var totalLessons = 21', c)
    return c
...
c = insert_common(c)
# (a) menu card: after the aula-20 menu <a>...</a> card.
mark = 'marlene-landucci-aula20.html?autostart=1'
mi = c.index(mark)
