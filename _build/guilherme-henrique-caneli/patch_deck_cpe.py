#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Troca o deck IN CLASS de UMA aula no arquivo do professor e espelha para o aluno.

Generalizacao do patch_deck_l2.py da aula 2, sem mudar o que ele faz: os mesmos tres
blocos trocados, as mesmas travas, o mesmo bloco de CSS. O que entra por argumento e
o numero da aula.

USO (da raiz): python3 _build/guilherme-henrique-caneli/patch_deck_cpe.py <n>
...
CSS_INI = '/* === CPE: inicio (gerado por patch_deck_cpe.py) === */'
CSS_FIM = '/* === CPE: fim === */'
...
.cpe-two .ic-chip { font-size:.71rem;line-height:1.22;padding:.22rem .4rem;margin-bottom:.16rem;border-radius:7px; }
...
.cpe-tight .ic-lf { font-size:.86rem;line-height:1.4;padding:.34rem 0; }
.cpe-tight .ic-lf .ic-lbl { font-size:.74rem; }
.cpe-tight .ic-bank { gap:.3rem;margin-top:.6rem; }
.cpe-tight .ic-b { font-size:.78rem;padding:.24rem .5rem; }
.cpe-tight .ic-card { padding:.9rem 1.1rem; }
/* 5 cartoes em 2 colunas sao 3 fileiras de 220px: nao cabe. Em 3 colunas sao 2. */
...
.cpe-tight .vocab-card .card-icon svg { width:18px;height:18px; }
...
def main():
    n = int(sys.argv[1])
    PROF = os.path.join(ROOT, 'public', 'professor', '%s-aula%d.html' % (SLUG, n))
    novo = open(os.path.join(ROOT, '_build', '%s-aula%d' % (SLUG, n), 'slides_cpe.html'),
                encoding='utf-8').read()
    ...
    print('  + aula %d, professor: %d -> %d slides' % (n, antes_slides, depois_slides))
