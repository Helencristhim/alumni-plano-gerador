...
| 08 | LEITURA | The Week the Rain Stopped Everything | Past simple: verbos irregulares de alta frequência (went, made, took, saw, had, said, got, came) | Texto curto sobre uma semana de imprevistos no canteiro + ordenar os fatos na sequência + true/false | Ouvir o texto e recontá-lo oralmente em 6 frases, no passado | rain, storm, mud, flood, stuck, delay, cover, decision, news |
...
> **Aula 08, duas trocas no vocabulário (15/09/2026, na geração).** Saíram `wait` e `finally`,
> entraram `storm` e `flood`. `wait` colidia com o vocab card *Wait* que a aula 10 ainda tem no hub
> (REGRA 22, gate `check_vocab_progression`), e `finally` já é ensinado na aula 03 como sequenciador.
> As duas substitutas são do mesmo campo da aula (a semana de chuva) e não aparecem em nenhuma outra.
