...
SNIP = open('_build/maria-claudia-curimbaba-aula17/hub_snippets.html', encoding='utf-8').read()
...
def process(path, is_prof):
    ...
    if is_prof:
        anchor = '<a href="/professor/maria-claudia-curimbaba-aula16.html?autostart=1"'
        card = card_raw.replace('aula17.html?autostart=1" ', 'aula17.html?autostart=1" target="_blank" ')
        assert anchor in html, f'{path}: IN CLASS aula16 anchor not found'
        html = insert_before(html, anchor, card.strip())

    # 2. stamp — before stamp16
    anchor = '<div class="stamp" id="stamp16"'
    assert anchor in html, f'{path}: stamp16 not found'
    html = insert_before(html, anchor, stamp_raw)

    # 3. accordion — before ex-lesson-16
    anchor = '<div class="lesson-card" id="ex-lesson-16">'
    assert anchor in html, f'{path}: ex-lesson-16 not found'
    html = insert_before(html, anchor, accord_raw)

    # 3b. complementary — before aula16 complementary header
    anchor = '<h4 style="font-family:\'Cormorant Garamond\',serif;font-size:1.1rem;color:var(--accent);margin-top:2rem;margin-bottom:.8rem">Aula 16 -- Contracts and Agreements</h4>'
    assert anchor in html, f'{path}: complementary aula16 header not found'
    ...
    html, n = re.subn(r'totalLessons=16', 'totalLessons=17', html)
    assert n >= 1, f'{path}: totalLessons=16 not found'
