#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 12 nos hubs prof e aluno de daniel-bastos.
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula11_link = '/professor/daniel-bastos-aula11.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula11_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula11 não encontrado ({kind})'
        a11_block = m.group(1)
        assert t.count(a11_block) == 1
        t = t.replace(a11_block, a11_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp11
    stamp11 = '<div class="stamp" id="stamp11" data-label="Acuerdo y Desacuerdo" style="background-image:url(\'https://images.unsplash.com/photo-1573497491208-6b1acb260507?w=200&q=80\')"></div>'
    t = insert_after(t, stamp11, '\n' + stamp, f'{kind} stamp')
    ...
    # 6. totalLessons 11 -> 12 (TOTAL_AULAS fica 48)
    t2 = t.replace('var totalLessons=11;', 'var totalLessons=12;')
