#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 12 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula11.

Aulas 1-11 intactas. Idempotente: se ex-lesson-12 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp12, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l12]) e ajusta totalLessons=12.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l12]": "/audio/carolina-paludetto-rodrigues/pc12_order_sequence.mp3",'
...
assert 'id="stamp12"' in stamp, stamp[:60]
assert 'id="ex-lesson-12"' in accordion, accordion[:60]
assert 'data-media="l12-youtube"' in comp, comp[:60]
assert amap_entries and 'a12_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp11 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]
    ...
    # 3. complementares l12 apos o ultimo wrapper de l11
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula11.html?autostart=1')
