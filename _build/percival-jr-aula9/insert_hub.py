...
amap_lines.append(f'  "[order-l9]": "{AUDIO_BASE}pc9_order_l9.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp9 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp9".*?</div>)', snip, re.S)
stamp9 = ms.group(1)

STAMP8 = ('<div class="stamp" id="stamp8" data-label="Future Plans" '
          "style=\"background-image:url('https://images.unsplash.com/"
          "photo-1506784983877-45594efa4cbe?w=200&q=80')\"></div>")

AULA8_HREF = 'percival-jr-aula8.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 2. stamp9 (apos stamp8 Future Plans)
    ...
    # 5. totalLessons 8 -> 9
    assert len(re.findall(r'totalLessons=8(?![0-9])', s)) == 1, f'{path}: totalLessons=8 nao unico'
    s = re.sub(r'totalLessons=8(?![0-9])', 'totalLessons=9', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 8
    if is_prof:
        i = s.index(AULA8_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
