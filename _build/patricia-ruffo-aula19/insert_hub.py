#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA dos snippets da aula 19 no hub prof + aluno de patricia-ruffo.
Idempotente: aborta se a aula 19 já estiver presente. Não toca aulas anteriores."""
import re, os, sys
...
COMP     = block('-- 3b. COMPLEMENTARES da aula 19 (inserir na tab-complementary, prof E aluno) -->', '<!-- 4. ENTRADAS')
...
def patch(path, is_prof):
    ...
    assert 'id="ex-lesson-19"' not in s, f'{path}: aula 19 JÁ presente — abortando'
    # 1. STAMP after stamp18
    anchor = '<div class="stamp" id="stamp18" data-label="Discussion Leader" style="background-image:url(\'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=200&q=80\')"></div>'
    assert anchor in s, f'{path}: anchor stamp18 não encontrado'
    ...
    # 3. IN CLASS card (só prof) — após o card da aula 18
    if is_prof:
        card18_end = '<div><div style="font-weight:600;font-size:.95rem">Leading a Research Discussion</div><div style="font-size:.8rem;color:var(--text-dim)">Chairing a meeting in English: opening the discussion, inviting contributions, keeping the group on track and wrapping up with clear action points -- 30 slides</div></div>\n    </a>'
        assert card18_end in s, f'{path}: card IN CLASS aula18 não encontrado'
        s = s.replace(card18_end, card18_end + '\n' + CARD, 1)
    ...
    # 6. totalLessons -> 19 (nunca rebaixar; maior nº presente)
    s2 = re.sub(r'var totalLessons=\d+;', 'var totalLessons=19;', s, count=1)
    assert s2 != s and 'var totalLessons=19;' in s2, f'{path}: totalLessons não encontrado'
    s = s2
