#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insercao ADITIVA dos snippets da aula 15 no hub prof + aluno de patricia-ruffo.
Idempotente: aborta se a aula 15 ja estiver presente. Nao toca aulas anteriores."""
import re, os, sys
...
COMP   = block('3b. COMPLEMENTARES da aula 15 (inserir na tab-complementary, prof E aluno) -->', '<!-- 4. ENTRADAS')
...
# anchors no hub (estado atual = aula 12 e a ultima)
STAMP12 = '<div class="stamp" id="stamp12" data-label="Data Storyteller" style="background-image:url(\'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=200&q=80\')"></div>'
...
def patch(path, is_prof):
    ...
    assert 'id="ex-lesson-15"' not in s, f'{path}: aula 15 JA presente -- abortando'
    # 1. STAMP after stamp12
    assert STAMP12 in s, f'{path}: anchor stamp12 nao encontrado'
    ...
    # 3. IN CLASS card (so prof) -- apos o card da aula 12
    if is_prof:
    ...
    # 6. totalLessons -> 15 (nunca rebaixar; hub atual = 12)
    s2 = re.sub(r'var totalLessons=\d+;', 'var totalLessons=15;', s, count=1)
    assert s2 != s and 'var totalLessons=15;' in s2, f'{path}: totalLessons nao encontrado'
    s = s2
