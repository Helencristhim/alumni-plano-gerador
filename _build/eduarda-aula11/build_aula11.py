...
# -*- coding: utf-8 -*-
"""
Build standalone Aula 11 (Eduarda Gabriel) — Chapter 11: Legal Framework Vocabulary.
Scaffold (CSS + JS engine + Planejamento tab) = the monolithic public/professor/eduarda-gabriel.html.
Reuses the proven slide engine. Fragments authored in _build/eduarda-aula11/*.html ; audio from phrases.json.
Outputs:
  public/professor/eduarda-gabriel-aula11.html  (4 tabs)
  public/aluno/eduarda-gabriel-aula11.html      (2 tabs)
...
PROF_OUT = os.path.join(ROOT, 'public', 'professor', 'eduarda-gabriel-aula11.html')
ALUNO_OUT= os.path.join(ROOT, 'public', 'aluno', 'eduarda-gabriel-aula11.html')
...
# ---- 2. JS block (the MAIN engine <script> — the one with var totalSlides) ----
...
js, n = re.subn(r'var lessonRanges = \{.*?\};',
                'var lessonRanges = { 11: {start: 1, end: %d} };' % N, js, flags=re.S)
assert n == 1, 'lessonRanges patch'

js, n = re.subn(r'var currentLesson = 1;', 'var currentLesson = 11;', js)
...
# ---- 2b. replace lesson-1 quick-challenge array with aula-11 Chapter 11 situations ----
CHALLENGES = '''var challenges = [
  { prompt: 'Explain the first step, using the passive. How does a Chapter 11 case begin?', answer: '"A petition is filed with the court."' },
  { prompt: 'Describe what protects the company from creditors.', answer: '"The company is protected by an automatic stay."' },
  { prompt: 'Say who keeps control of the business.', answer: '"The firm stays in control as debtor in possession."' },
  { prompt: 'State the obligation about the reorganization plan.', answer: '"The plan must be approved by the court."' },
  { prompt: 'Explain what happens to some debts at the end.', answer: '"Some debts are discharged at the end of the case."' },
  { prompt: 'Describe the company financial state before filing.', answer: '"The company became insolvent and had to file for bankruptcy."' }
];
...
STAMPS = ('<div class="stamps-row">\n'
  ...
  '<div class="stamp" id="stamp11" data-label="Chapter 11" style="background-image:url(\'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=200&q=80\')"></div>\n'
  '</div>')
...
def header(badge, hubhref):
    return ('<body>\n\n'
      ...
      '      <div class="progress-label"><span>Aula 11 &mdash; Chapter 11: Legal Framework Vocabulary</span><span id="progressPercent">0%</span></div>\n'
...
WELCOME = '''<div class="exercise-section" style="margin-bottom:1.5rem">
  ...
  <p style="font-size:.88rem;line-height:1.7;margin-bottom:1rem">This week you build the legal language of restructuring: the core <strong>Chapter 11</strong> vocabulary and the <strong>passive voice</strong> used to describe legal processes. Review these key phrases before class.</p>
  <div style="display:grid;gap:0.8rem">
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">The petition was filed with the court.</strong><button class="audio-btn" onclick="speakText('The petition was filed with the court.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">The company is protected by an automatic stay.</strong><button class="audio-btn" onclick="speakText('The company is protected by an automatic stay.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">The reorganization plan must be approved by the court.</strong><button class="audio-btn" onclick="speakText('The reorganization plan must be approved by the court.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">Some debts are discharged at the end of the case.</strong><button class="audio-btn" onclick="speakText('Some debts are discharged at the end of the case.', this)">Listen</button></div>
  </div>
</div>'''
...
INCLASS_MENU = '''<div class="tab-content" id="tab-inclass">
  <h3 style="font-family:'Cormorant Garamond',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 11</h3>
  <div style="display:flex;flex-direction:column;gap:1rem">
    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" onclick="enterSlideMode(1)" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='rgba(200,200,190,.5)'">
      ...
      <div><div style="font-weight:600;font-size:.95rem">Chapter 11: Legal Framework Vocabulary</div><div style="font-size:.8rem;color:var(--text-dim)">Legal collocations &amp; passive voice &mdash; ''' + str(N) + ''' slides</div></div>
    </div>
  </div>
</div>'''
...
AUDIT = '''<!--
SCENARIO FIT — Aula 11 (Chapter 11 — Legal Framework Vocabulary)
Can-do: "I can use the core Chapter 11 vocabulary and describe a restructuring process in English using the passive voice."
Gramatica-alvo: passive voice (be + past participle), incl. present/past/modal passives — habitat natural do ingles juridico-financeiro.
Vocab-alvo (NOVO): file for bankruptcy, automatic stay, debtor in possession, reorganization plan, discharge, insolvent, creditors' committee, court approval.
Cenario escolhido: Eduarda e briefada por David (advogado de reestruturacao) sobre um caso de Chapter 11 e depois explica o processo a um colega/cliente.
Por que elicita o alvo: descrever um processo legal passo a passo OBRIGA o uso da passiva ("a petition is filed", "an automatic stay is granted", "the plan must be approved") + o vocabulario tecnico. >70% dos itens-alvo sao elicitados naturalmente. E a aula "Chapter 11 — Legal Framework Vocabulary / Legal collocations + passive voice" do curriculo.

CONTINUIDADE — Aula 11
Itens NOVOS: os 8 termos juridicos acima + passive voice.
Itens revisados (aulas anteriores): resumir uma reuniao (Aula 9/10) e opiniao com dados (Aula 8) sao reativados no warm-up (slide 2) como ponte; o vocabulario de reestruturacao da Aula 4 e PRESSUPOSTO, nao reintroduzido (REGRA 22).
Callback no warm-up (slide 2): o aluno (1) resume uma decisao de uma reuniao de estrategia (Aula 10) e (2) so entao faz a ponte para a estrategia escolhida — protocolar o Chapter 11 — que exige o vocabulario juridico de hoje.
NB: vocabulario novo distinto das aulas 1-10. A Aula 4 ensinou colocacoes gerais de restructuring; a Aula 11 ensina termos JURIDICOS especificos do Chapter 11 (nao re-ensinados como novos).
-->
'''

# ---- assemble PROFESSOR ----
prof = (
    head('Professor View &mdash; Eduarda Gabriel | Aula 11 | Chapter 11: Legal Framework | Alumni by Better')
...
aluno = (
    head('Aluno &mdash; Eduarda Gabriel | Aula 11 | Chapter 11: Legal Framework | Alumni by Better')
