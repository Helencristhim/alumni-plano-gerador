#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 14 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula13.

Aulas 1-13 intactas. Idempotente: se ex-lesson-14 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp14, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l14]) e ajusta totalLessons=14.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l14]": "/audio/carolina-paludetto-rodrigues/pc14_order_sequence.mp3",'
...
assert 'id="stamp14"' in stamp, stamp[:60]
assert 'id="ex-lesson-14"' in accordion, accordion[:60]
assert 'data-media="l14-youtube"' in comp, comp[:60]
assert amap_entries and 'a14_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp13 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]

    # 2. accordion ex-lesson-14 apos ex-lesson-13
    st = s.index('<div class="lesson-card" id="ex-lesson-13">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l14 apos o ultimo wrapper de l13
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula13.html?autostart=1')
