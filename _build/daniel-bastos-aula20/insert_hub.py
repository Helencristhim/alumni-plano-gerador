#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 20 nos hubs prof e aluno de daniel-bastos."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula19_link = '/professor/daniel-bastos-aula19.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula19_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula19 não encontrado ({kind})'
        a19_block = m.group(1)
        assert t.count(a19_block) == 1
        t = t.replace(a19_block, a19_block + '\n' + menu_card, 1)

    stamp19 = '<div class="stamp" id="stamp19" data-label="Hoteles y Registro" style="background-image:url(\'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=200&q=80\')"></div>'
    t = insert_after(t, stamp19, '\n' + stamp, f'{kind} stamp')
    ...
    t2 = t.replace('var totalLessons=19;', 'var totalLessons=20;')
