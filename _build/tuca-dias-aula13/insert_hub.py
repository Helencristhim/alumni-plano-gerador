#!/usr/bin/env python3
# Insere os snippets da aula 13 nos hubs prof + aluno (SO ADITIVO).
import re, sys

SNIP = open('_build/tuca-dias-aula13/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/tuca-dias-aula12.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('tuca-dias-aula13.html?autostart') == 0, 'aula13 card ja existe'
    ...
    # --- 3. ACCORDION Pre-class (after ex-lesson-12, before next major block) ---
