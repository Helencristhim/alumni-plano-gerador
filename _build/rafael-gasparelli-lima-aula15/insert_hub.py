#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insercao ADITIVA dos snippets da aula 15 no hub existente (prof + aluno).
Apenas insere por ancora de string; aulas 1-14 ficam intactas."""
import re, os

ROOT = '/home/dan/dev/work/better/wt-rafgasp-gen-a15'
SNIP = os.path.join(ROOT, '_build/rafael-gasparelli-lima-aula15/hub_snippets.html')
snip = open(SNIP, encoding='utf-8').read()

# --- extrair blocos do snippet ---
# Accordion: do <div class="lesson-card" id="ex-lesson-15"> ate o marcador 3b
acc_start = snip.index('<div class="lesson-card" id="ex-lesson-15">')
acc_end_marker = '<!-- 3b. COMPLEMENTARES'
accordion = snip[acc_start:snip.index(acc_end_marker)].rstrip() + '\n'

# Complementares: do primeiro media-card-wrapper l15 ate o marcador 4.
comp_start = snip.index('<div class="media-card-wrapper" data-media="l15-series">')
...
# Stamp
stamp = '''        <div class="stamp" id="stamp15" data-label="Up to Now" style="background-image:url('https://images.unsplash.com/photo-1507925921958-8a62f3d1a50d?w=200&q=80')"></div>'''

# Menu card no formato do hub (inclass-lesson-card) — uniforme com aulas anteriores
menu_card = '''<a href="/professor/rafael-gasparelli-lima-aula15.html?autostart=1" class="inclass-lesson-card" style="text-decoration:none;color:inherit">
  ...
  <div class="ilc-info">
    <div class="ilc-number">LESSON 15 &mdash; 60 MINUTES</div>
    <div class="ilc-title">Up to Now &mdash; Present Perfect</div>
    <div class="ilc-desc">Present Perfect, have/has + participle &mdash; Lesson 15 &mdash; 28 slides</div>
  </div>
...
def process(path, is_prof):
    ...
    orig_len = len(s)
    # guard: nao reinserir
    assert 'id="ex-lesson-15"' not in s, f'{path} ja tem aula 15'

    # 1. audioMap (apos 'var audioMap = {\n')
    s = s.replace('var audioMap = {\n', 'var audioMap = {\n' + audiomap_entries, 1)

    # 2. stamp (apos a linha do stamp14)
    s = re.sub(r'(<div class="stamp" id="stamp14"[^\n]*\n)', r'\1' + stamp + '\n', s, count=1)
    ...
    s = s.replace('var totalLessons=14;', 'var totalLessons=15;', 1)
