#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 8 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp8, accordion Pre-class (ex-lesson-8), card IN CLASS (só prof),
bloco Complementares (l8), entradas de audioMap, e totalLessons 7->8.
NÃO toca em nada das aulas 1-7. Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula8.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 1 Review &#8212; The Full Meeting</div><div style="font-size:.8rem;color:var(--text-dim)">Running a Meeting End-to-End &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-8"' not in s, f'{path}: aula 8 já integrada'
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=7;', '  var totalLessons=8;', 'totalLessons')
    # 6. audioMap entries
