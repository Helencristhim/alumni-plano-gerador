# -*- coding: utf-8 -*-
"""Insere os snippets da aula 18 nos hubs PROF e ALUNO da Simone (aditivo).
Adiciona o stamp18 (ainda nao existe no hub) e ajusta totalLessons para 18."""
import os, re
...
audio_lines += f'\n  "[order-l18]": "{AUDIO_BASE}a18_order_sequence.mp3",'

# menu card (snippet section 1)
mc = re.search(r'1\. CARD.*?-->\n(.*?)\n\n<!-- 2\.', snip, re.S)
menu_card = mc.group(1).rstrip('\n')

# stamp18 (snippet section 2)
ms = re.search(r'2\. STAMP.*?-->\n(.*?)\n\n<!-- 3\.', snip, re.S)
stamp18 = ms.group(1).rstrip('\n')
assert 'id="stamp18"' in stamp18, 'stamp18 ausente no snippet (config tem stamp id 18?)'

def patch(path, is_prof):
    s = open(path, encoding='utf-8').read()
    orig_len = len(s)
    assert 'id="ex-lesson-18"' not in s, f'{path}: aula 18 ja inserida'
    assert 'id="stamp18"' not in s, f'{path}: stamp18 ja existe'
    assert 'id="ex-lesson-17"' in s, f'{path}: aula 17 ausente (hub desatualizado?)'
    ...
    # 5) stamp18 na stamps-row (apos stamp17)
    m17 = re.search(r'(<div class="stamp" id="stamp17"[^>]*></div>)', s)
    assert m17, f'{path}: stamp17 ausente na stamps-row'
    s = s.replace(m17.group(1), m17.group(1) + '\n' + stamp18, 1)

    # 6) totalLessons -> 18 (NAO mexer em TOTAL_AULAS=48, que e o total do programa)
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 18', s)
