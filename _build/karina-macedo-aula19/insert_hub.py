#!/usr/bin/env python3
"""Insere a aula 19 nos hubs prof e aluno da Karina -- SO ADITIVO.
...
def patch_hub(path, is_prof):
    s = read(path)
    n0 = len(s)
    assert 'ex-lesson-19' not in s, 'aula 19 JA esta no hub -- abortando (idempotencia)'

    # 1. STAMP -- apos o stamp18
    stamp18 = '<div class="stamp" id="stamp18"'
    end18 = s.index('</div>', s.index(stamp18)) + len('</div>')
    s = s[:end18] + '\n' + stamp + s[end18:]

    # 2. ACCORDION Pre-class -- apos o fechamento do ex-lesson-18
    anchor18 = '<div class="lesson-card" id="ex-lesson-18">'
    p18 = s.index(anchor18)
    nxt = s.find('<div class="lesson-card"', p18 + len(anchor18))
    tab_end = s.find('</div><!-- /tab-exercises -->', p18)
    ...
    # 3. COMPLEMENTARES -- apos o ultimo card l18 (l18-youtube wrapper)
    yt = '<div class="media-card-wrapper" data-media="l18-youtube">'
    pyt = s.index(yt)
    ...
    if comp_tab_end != -1:
        ...
        assert m, 'nao achei fim do wrapper l18-youtube'
        end = pyt + m.end()
        s = s[:end] + '\n' + comp + s[end:]
    ...
    assert 'var totalLessons=18' in s, 'nao achei var totalLessons=18'
    s = s.replace('var totalLessons=18', 'var totalLessons=19')

    # 6 (so prof). IN CLASS card -- apos o card da aula 18
    if is_prof:
        card_anchor = '/professor/karina-macedo-aula18.html?autostart=1'
        pc = s.index(card_anchor)
        a_end = s.index('</a>', pc) + len('</a>')
        s = s[:a_end] + '\n' + card + s[a_end:]

    assert 'ex-lesson-19' in s and 'stamp19' in s and 'l19-series' in s, 'pos-condicao falhou'
    assert 'var totalLessons=19' in s
    if is_prof:
        assert 'karina-macedo-aula19.html?autostart=1' in s
    write(path, s)
