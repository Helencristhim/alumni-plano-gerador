#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 8 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-7 intactas. Idempotente: se ex-lesson-8 ja existe, aborta.
Molde aula7: ancoras stamp7/ex-lesson-7/l7-youtube/aula7.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp8" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l8]": "/audio/erica-maria-machado-santarem/pc8_order_l8.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp8 apos stamp7 — SO se ainda nao existir
    if 'id="stamp8"' not in s:
        m = re.search(r'<div class="stamp" id="stamp7"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp8 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula7.html?autostart=1')
