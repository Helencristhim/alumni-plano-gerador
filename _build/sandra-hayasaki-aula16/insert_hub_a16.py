#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 16 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp16, (3) ex-lesson-16 accordion,
(4) complementary aula 16 block, (5) audioMap entries; bumps totalLessons 15->16.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-16"' in html and 'id="stamp16"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 16 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula15 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula15.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp15 div (within stamps-row)
    sm = 'id="stamp15"'
    si = html.index(sm)
    ...
    # 4. COMPLEMENTARY -- insert the aula-16 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=15;', 'var totalLessons=16;')
    if html2 != html:
