#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 13 nos hubs (prof + aluno) de patricia-xavier-lima-iwashita
de forma ADITIVA. Aulas 1-12 intactas. Idempotente: se ex-lesson-13 ja existe, aborta.
Ancora na aula 12 (stamp12/ex-lesson-12/l12-youtube/aula12 card). Sem order-audio."""
import os, re
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: stamp12 nao encontrado'
    s = s[:m.end()] + '\n' + stamp + s[m.end():]
    ...
    if is_prof:
        ci = s.index('patricia-xavier-lima-iwashita-aula12.html?autostart=1')
...
patch(os.path.join(ROOT, 'public', 'professor', 'patricia-xavier-lima-iwashita.html'), True)
patch(os.path.join(ROOT, 'public', 'aluno', 'patricia-xavier-lima-iwashita.html'), False)
print('OK')
