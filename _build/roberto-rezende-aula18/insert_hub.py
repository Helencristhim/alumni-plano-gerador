#!/usr/bin/env python3
# Insere os snippets da aula 18 nos hubs prof + aluno do roberto-rezende (SO ADITIVO).
# Legado: hub aluno tem buracos (ex-lesson 7-9, stamps 7-11 faltando), PRE-EXISTENTE.
# NAO consertamos: so anexamos a aula 18 apos o ULTIMO stamp/ex-lesson presente em CADA arquivo.
import re

SNIP = open('_build/roberto-rezende-aula18/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    # --- 1. MENU CARD (prof only) — apos o card da aula 17 ---
    if is_prof:
        anchor = '/professor/roberto-rezende-aula17.html?autostart=1'
        assert h.count('roberto-rezende-aula18.html?autostart') == 0, 'aula18 card ja existe'
        i = h.index(anchor)
    ...
    # --- 4. COMPLEMENTARY (apos o ULTIMO wrapper l17, dentro de tab-complementary) ---
    assert 'data-media="l18-' not in h, 'l18 complementary ja existe'
    last_l17 = [m.start() for m in re.finditer(r'data-media="l17-', h)][-1]
    wrap_end = end_of_wrapper(h, last_l17)
    h = h[:wrap_end] + '\n' + comp + '\n' + h[wrap_end:]
    ...
    # --- 6. totalLessons 17 -> 18 (ADITIVO) ---
    h2 = re.sub(r'var totalLessons ?= ?17', 'var totalLessons=18', h)
