...
SNIP = open('_build/aline-sberci-aula16/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/aline-sberci-aula15.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('aline-sberci-aula16.html?autostart') == 0, 'aula16 card ja existe'
    ...
    # --- 4. COMPLEMENTARY (after the l15 media-grid close, as a sibling inside tab-complementary) ---
    assert 'data-media="l16-' not in h, 'l16 complementary ja existe'
    ci = h.index('data-media="l15-podcast"')
    # the l15 media-grid closes with '</div>\n</div>\n' right after the last wrapper
