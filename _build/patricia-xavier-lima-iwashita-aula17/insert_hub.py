#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 nos hubs (prof + aluno) de patricia-xavier-lima-iwashita
de forma ADITIVA. Aulas 1-16 intactas. Idempotente: se ex-lesson-17 ja existe, aborta.
Ancora na aula 16 (stamp16/ex-lesson-16/l16-youtube/aula16 card). Sem order-audio."""
import os, re
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: stamp16 nao encontrado'
    s = s[:m.end()] + '\n' + stamp + s[m.end():]
    ...
    if is_prof:
        ci = s.index('patricia-xavier-lima-iwashita-aula16.html?autostart=1')
