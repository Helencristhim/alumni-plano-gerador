#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 16 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-15 intactas. Idempotente: se ex-lesson-16 ja existe, aborta.
Molde aula15: ancoras stamp15/ex-lesson-15/l15-youtube/aula15.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp16" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l16]": "/audio/erica-maria-machado-santarem/pc16_order_l16.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp16 apos stamp15 — SO se ainda nao existir
    if 'id="stamp16"' not in s:
        m = re.search(r'<div class="stamp" id="stamp15"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp16 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    # 3. complementares l16 apos o ultimo wrapper l15
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula15.html?autostart=1')
