...
SNIP = open('_build/maria-claudia-curimbaba-aula20/hub_snippets.html', encoding='utf-8').read()
...
def process(path, is_prof):
    ...
    if is_prof:
        anchor = '<a href="/professor/maria-claudia-curimbaba-aula19.html?autostart=1"'
        card = card_raw.replace('aula20.html?autostart=1" ', 'aula20.html?autostart=1" target="_blank" ')
        assert anchor in html, f'{path}: IN CLASS aula19 anchor not found'
        html = insert_before(html, anchor, card.strip())

    # 2. stamp — before stamp19
    anchor = '<div class="stamp" id="stamp19"'
    assert anchor in html, f'{path}: stamp19 not found'
    html = insert_before(html, anchor, stamp_raw)

    # 3. accordion — before ex-lesson-19
    anchor = '<div class="lesson-card" id="ex-lesson-19">'
    assert anchor in html, f'{path}: ex-lesson-19 not found'
    html = insert_before(html, anchor, accord_raw)

    # 3b. complementary — before aula19 complementary header
    anchor = '<h4 style="font-family:\'Cormorant Garamond\',serif;font-size:1.1rem;color:var(--accent);margin-top:2rem;margin-bottom:.8rem">Aula 19 -- Houston, We Have Culture: US vs Brazil</h4>'
    assert anchor in html, f'{path}: complementary aula19 header not found'
    ...
    html, n = re.subn(r'totalLessons=19', 'totalLessons=20', html)
    assert n >= 1, f'{path}: totalLessons=19 not found'
