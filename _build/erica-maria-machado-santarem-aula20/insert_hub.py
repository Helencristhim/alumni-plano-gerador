#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 20 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-19 intactas. Idempotente: se ex-lesson-20 ja existe, aborta.
Ancoras: stamp19/ex-lesson-19/l19-youtube/aula19.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp20" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l20]": "/audio/erica-maria-machado-santarem/pc20_order_l20.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp20 apos stamp19 — SO se ainda nao existir
    if 'id="stamp20"' not in s:
        m = re.search(r'<div class="stamp" id="stamp19"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp20 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula19.html?autostart=1')
