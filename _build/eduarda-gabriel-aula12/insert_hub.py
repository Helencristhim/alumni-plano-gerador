#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 12 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-12 ja existe, aborta.
Molde: eduarda-gabriel-aula11/insert_hub.py. Ancora de complementares = l11-youtube
(a aula 11 introduziu o prefixo 'youtube' como 3a midia)."""
import os, re
...
ORDER_AMAP = '  "[order-l12]": "/audio/eduarda-gabriel/order_l12_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-11">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l12 apos o ultimo wrapper l11 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula11.html?autostart=1')
