#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 11 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde diogo-leal-aula8.

Aulas 1-10 intactas. Idempotente: se ex-lesson-11 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp11, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l11]) e ajusta totalLessons=11.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l11]": "/audio/carolina-paludetto-rodrigues/pc11_order_sequence.mp3",'
...
assert 'id="stamp11"' in stamp, stamp[:60]
assert 'id="ex-lesson-11"' in accordion, accordion[:60]
assert 'data-media="l11-youtube"' in comp, comp[:60]
assert amap_entries and 'a11_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp10 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]
    ...
    # 3. complementares l11 apos o ultimo wrapper de l10
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula10.html?autostart=1')
