#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 8 nos hubs prof e aluno de helio-santana.
Extrai as 5 seções do hub_snippets.html e as encaixa por âncora de string.
NÃO toca nas aulas 1-7 — só APPEND."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula7_link = '/professor/helio-santana-aula7.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula7_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula7 não encontrado ({kind})'
        a7_block = m.group(1)
        assert t.count(a7_block) == 1, f'card aula7 ambíguo ({kind})'
        t = t.replace(a7_block, a7_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp7
    stamp7 = '<div class="stamp" id="stamp7" data-label="Presentar la Empresa" style="background-image:url(\'https://images.unsplash.com/photo-1497366216548-37526070297c?w=200&q=80\')"></div>'
    assert stamp7 in t, f'âncora stamp7 não encontrada ({kind})'
    assert t.count(stamp7) == 1, f'stamp7 ambíguo ({kind})'
    t = t.replace(stamp7, stamp7 + '\n' + stamp, 1)

    # 3. ACCORDION -> antes do fechamento da tab-exercises (após o ex-lesson-7)
    ...
    # 6. totalLessons 7 -> 8 (TOTAL_AULAS fica 50)
    t2 = t.replace('var totalLessons=7', 'var totalLessons=8')
