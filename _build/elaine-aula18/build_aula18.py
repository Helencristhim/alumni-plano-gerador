#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build standalone Aula 18 (Elaine) -- Taking Photos: Asking Strangers. Splices authored
content into the proven aula9 scaffold (CSS+JS verbatim). Mirrors build_aula17.py.
...
a aula ENTREGUE e a INLINE no monolitico (ver inline_18.py)."""
import os, re, json
...
PROF_OUT  = os.path.join(REFDIR, 'elaine-mieko-pinho-aula18.html')
ALUNO_OUT = os.path.join(REFDIR, 'elaine-mieko-pinho-aula18-aluno.html')
...
def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 18 | Taking Photos: Asking Strangers')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 18</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Taking Photos -- Asking Strangers</div><div style="font-size:.8rem;color:var(--text-dim)">Asking a stranger for a photo -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
...
print('OK aula18 standalone (ref) built; audioMap keys', len(phrases))
