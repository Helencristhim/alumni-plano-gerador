#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 13 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp13, (3) ex-lesson-13 accordion,
(4) complementary aula 13 block, (5) audioMap entries; bumps totalLessons 12->13.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-13"' in html and 'id="stamp13"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 13 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula12 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula12.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp12 div (within stamps-row)
    sm = 'id="stamp12"'
    si = html.index(sm)
    ...
    html2 = html.replace('var totalLessons=12;', 'var totalLessons=13;')
    if html2 != html:
