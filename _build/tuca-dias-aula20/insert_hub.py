...
SNIP = open('_build/tuca-dias-aula20/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/tuca-dias-aula19.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('tuca-dias-aula20.html?autostart') == 0, 'aula20 card ja existe'
    ...
    # --- 3. ACCORDION Pre-class (after ex-lesson-19, before next major block) ---
    ...
    ci = h.index('data-media="l19-youtube"')
