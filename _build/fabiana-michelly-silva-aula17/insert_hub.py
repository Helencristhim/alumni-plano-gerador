#!/usr/bin/env python3
"""Inserção ESTRITAMENTE ADITIVA da aula 17 nos hubs prof + aluno da Fabiana.
Âncoras de string inequívocas. Idempotente: aborta se a17 já existe."""
import os
...
def patch(path, is_prof):
    ...
    if 'ex-lesson-17' in html or 'stamp17' in html:
        print(f'  SKIP {os.path.basename(path)} — aula17 já presente'); return
    orig = html

    # STAMP: após o </div> do stamp16
    a = '<div class="stamp" id="stamp16"'
    ...
    # COMPLEMENTARES: após o wrapper l16-youtube
    pos = end_of_wrapper(html, html.index('data-media="l16-youtube"'))
    html = html[:pos] + '\n' + COMP + html[pos:]

    # MENU IN CLASS (só prof): após o </a> do card da aula 16
    if is_prof:
        m = html.index('</a>', html.index('aula16.html?autostart=1')) + len('</a>')
        html = html[:m] + '\n    ' + MENU.strip() + html[m:]
    ...
    # totalLessons 16 -> 17
    html = html.replace('var totalLessons = 16;', 'var totalLessons = 17;')

    assert html != orig and 'ex-lesson-17' in html, 'inserção falhou'
