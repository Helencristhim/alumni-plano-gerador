#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere a aula 19 nos hubs de luiz-bressane de forma ADITIVA.
...
Idempotente: se stamp19 ja existe, aborta. Aulas 1-18 intactas."""
import os, re
...
CARD = (
    "<div class=\"inclass-lesson-card\" onclick=\"window.location.href='/professor/luiz-bressane-aula19.html?autostart=1'\" style=\"background:#fff;border:2px solid var(--accent);border-radius:12px;padding:1.5rem;cursor:pointer;transition:all .3s\">\n"
    "<div style=\"font-size:.7rem;text-transform:uppercase;letter-spacing:2px;color:var(--accent);font-weight:700;margin-bottom:.4rem\">Aula 19</div>\n"
    "<h3 style=\"font-family:'Cormorant Garamond',serif;font-size:1.3rem;color:var(--text);margin-bottom:.4rem\">After the Verdict</h3>\n"
    "<p style=\"font-size:.82rem;color:var(--text-dim)\">Result &amp; Consequence Linkers &middot; Explaining the Outcome</p>\n"
    "</div>"
)
...
def patch(path, is_prof):
    ...
    # 2. card IN CLASS (so prof) apos o card da aula 18
    if is_prof:
        ci = s.index("luiz-bressane-aula18.html?autostart=1")
