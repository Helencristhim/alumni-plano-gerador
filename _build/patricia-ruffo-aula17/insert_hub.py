#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA dos snippets da aula 17 no hub prof + aluno de patricia-ruffo.
Idempotente: aborta se a aula 17 já estiver presente. Não toca aulas anteriores."""
import re, os, sys
...
COMP     = block('-- 3b. COMPLEMENTARES da aula 17 (inserir na tab-complementary, prof E aluno) -->', '<!-- 4. ENTRADAS')
...
def patch(path, is_prof):
    ...
    assert 'id="ex-lesson-17"' not in s, f'{path}: aula 17 JÁ presente — abortando'
    ...
    # 3. IN CLASS card (só prof) — após o card da aula12
    if is_prof:
    ...
    # 6. totalLessons -> 17 (nunca rebaixar; maior nº presente)
    s2 = re.sub(r'var totalLessons=\d+;', 'var totalLessons=17;', s, count=1)
    assert s2 != s and 'var totalLessons=17;' in s2, f'{path}: totalLessons não encontrado'
    s = s2
