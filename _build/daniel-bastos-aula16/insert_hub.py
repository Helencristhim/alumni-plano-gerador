#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 16 nos hubs prof e aluno de daniel-bastos."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula15_link = '/professor/daniel-bastos-aula15.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula15_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula15 não encontrado ({kind})'
        a15_block = m.group(1)
        assert t.count(a15_block) == 1
        t = t.replace(a15_block, a15_block + '\n' + menu_card, 1)

    stamp15 = '<div class="stamp" id="stamp15" data-label="Negociar Acuerdos" style="background-image:url(\'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=200&q=80\')"></div>'
    t = insert_after(t, stamp15, '\n' + stamp, f'{kind} stamp')
    ...
    t2 = t.replace('var totalLessons=15;', 'var totalLessons=16;')
