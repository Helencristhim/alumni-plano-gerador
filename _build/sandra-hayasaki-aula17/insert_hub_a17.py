#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 17 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp17, (3) ex-lesson-17 accordion,
(4) complementary aula 17 block, (5) audioMap entries; bumps totalLessons 16->17.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-17"' in html and 'id="stamp17"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 17 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula16 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula16.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp16 div (within stamps-row)
    sm = 'id="stamp16"'
    si = html.index(sm)
    ...
    # 4. COMPLEMENTARY -- insert the aula-17 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=16;', 'var totalLessons=17;')
    if html2 != html:
