#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mklesson.py — AUTORIA das aulas da Luiza Sato Eccheli (B2, 80 aulas; este lote gera 1-10).

NAO substitui o builder (REGRA 20). Emite so os arquivos de CONTEUDO que se escreveriam a
mao em _build/luiza-sato-eccheli-aula{N}/ (slides.html, preclass.html,
...
aula em specs/aulaN.py. Depois o fluxo e o de sempre:
    ...
    python3 _build/model/build_from_model.py _build/luiza-sato-eccheli-aula{N}/config.json

POR QUE EXISTE. As aulas tem as MESMAS duas formas (aula impar = fala, com dialogo
...
aprovadas pelos gates, via o mklesson.py do Guilherme Henrique Caneli (14/09/2026), de
onde este arquivo foi copiado trocando so as constantes da aluna; o que muda por aula e
so a ficha. Escrever 10 vezes o esqueleto a
mao e o caminho conhecido para a REGRA 11 item 9 (uniformidade) quebrar na aula 6.

USO:  python3 _build/luiza-sato-eccheli/mklesson.py 1
"""
...
SLUG = 'luiza-sato-eccheli'
STUDENT = 'Luiza Sato Eccheli'
FIRST = 'Luiza'
TOTAL = 80
PALETTE = {'accent': '#6B3A5B', 'accent_light': '#B97AA3'}
ACCENT_RGB = '107,58,91'
HEADER = ['B2 (Intermedi&#225;rio Superior)', '18 anos &middot; Direito na FGV',
          'Pen&#225;polis e S&#227;o Paulo, SP',
          '60 min &middot; Online &middot; 1x por semana']
PROGRAM = 'English for the TOEFL and Global Law'
HUB_SUBTITLE = ('Ingl&#234;s para o TOEFL do interc&#226;mbio da FGV &mdash; fala sem travar nas palavras, '
                'leitura jur&#237;dica na velocidade do portugu&#234;s e o vocabul&#225;rio do direito global')
DARK = 'linear-gradient(rgba(36,16,30,.80),rgba(36,16,30,.92))'
...
def render_slides(L):
    ...
    def roleplays(ph):
        ...
        bgs = ['linear-gradient(135deg,var(--accent-dim),rgba(%s,.05))' % ACCENT_RGB,
               'linear-gradient(135deg,rgba(%s,.08),rgba(%s,.02))' % (ACCENT_RGB, ACCENT_RGB),
               'linear-gradient(135deg,rgba(%s,.12),rgba(%s,.03))' % (ACCENT_RGB, ACCENT_RGB)]
    ...
    if not reading:
        ...
        for i, (who, txt) in enumerate(d['lines']):
            key = 'luiza' if who == 'l' else L['guest_key']
            voice = 'ellen' if who == 'l' else L['guest_voice']
            ini = 'L' if who == 'l' else d['guest_initial']
...
def render_config(L):
    ...
    lesson['extra_audio'] = []
    cfg = {
        'slug': SLUG, 'molde': 'helen-mendes', 'student_name': STUDENT, 'first_name': FIRST, 'gender': 'f',
        'program': PROGRAM, 'total_aulas': TOTAL, 'palette': PALETTE, 'header': HEADER, 'hub_subtitle': HUB_SUBTITLE,
        'characters': {'luiza': 'ellen', L['guest_key']: L['guest_voice']},
...
STAMPS = [
    {'id': 1, 'label': 'The Application Story', 'img': 'https://images.unsplash.com/photo-1541339907198-e08756dedf3f?w=200&q=80'},
    {'id': 2, 'label': 'The Treaty Is Not the Problem', 'img': 'https://images.unsplash.com/photo-1505664194779-8beaceb93744?w=200&q=80'},
    {'id': 3, 'label': 'Say It Once, Clearly', 'img': 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=200&q=80'},
    {'id': 4, 'label': 'Who Judges a State?', 'img': 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=200&q=80'},
    {'id': 5, 'label': 'A Semester Abroad', 'img': 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?w=200&q=80'},
    {'id': 6, 'label': 'How the TOEFL Reads You', 'img': 'https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=200&q=80'},
    {'id': 7, 'label': 'The Tutorial Voice', 'img': 'https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=200&q=80'},
    {'id': 8, 'label': 'The Fine Print You Already Signed', 'img': 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=200&q=80'},
    {'id': 9, 'label': 'Forty-Five Seconds of Opinion', 'img': 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=200&q=80'},
    {'id': 10, 'label': 'The First Draft', 'img': 'https://images.unsplash.com/photo-1488190211105-8b0e65b80b4e?w=200&q=80'},
]
...
def check_spec(L):
    V = L['vocab']
    assert 12 <= len(V) <= 15, 'B2: 12-15 palavras por aula (REGRA 13), veio %d' % len(V)
    if L['model'] == 'speaking':
        assert L['guest_voice'] == 'arthur', 'a Luiza e ellen: o convidado do dialogo precisa de outra voz'
