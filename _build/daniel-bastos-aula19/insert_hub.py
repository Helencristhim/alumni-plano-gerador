#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 19 nos hubs prof e aluno de daniel-bastos."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula18_link = '/professor/daniel-bastos-aula18.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula18_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula18 não encontrado ({kind})'
        a18_block = m.group(1)
        assert t.count(a18_block) == 1
        t = t.replace(a18_block, a18_block + '\n' + menu_card, 1)

    stamp18 = '<div class="stamp" id="stamp18" data-label="Aeropuertos y Vuelos" style="background-image:url(\'https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=200&q=80\')"></div>'
    t = insert_after(t, stamp18, '\n' + stamp, f'{kind} stamp')
    ...
    t2 = t.replace('var totalLessons=18;', 'var totalLessons=19;')
