...
NAO toca aulas 1-13. So adiciona: card IN CLASS (so prof), stamp14, accordion
Pre-class, bloco de Complementares, entradas de audioMap (+ [order-l14]) e
ajusta var totalLessons 13->14. Fragmentos vem dos arquivos do builder.
"""
import os
import re
...
amap_lines.append(f'  "[order-l14]": "{AUDIO_BASE}pc14_order_l14.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp14 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp14".*?</div>)', snip, re.S)
stamp14 = ms.group(1)

STAMP13 = ('<div class="stamp" id="stamp13" data-label="Action Items" '
           "style=\"background-image:url('https://images.unsplash.com/"
           "photo-1484480974693-6ca0a78fb36b?w=200&q=80')\"></div>")

AULA13_HREF = 'percival-jr-aula13.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 2. stamp14 (apos stamp13 Action Items)
    ...
    assert len(re.findall(r'totalLessons=13(?![0-9])', s)) == 1, f'{path}: totalLessons=13 nao unico'
    s = re.sub(r'totalLessons=13(?![0-9])', 'totalLessons=14', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 13
    if is_prof:
        i = s.index(AULA13_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
