#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 18 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp18, (3) ex-lesson-18 accordion,
(4) complementary aula 18 block, (5) audioMap entries; bumps totalLessons 17->18.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-18"' in html and 'id="stamp18"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 18 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula17 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula17.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp17 div (within stamps-row)
    sm = 'id="stamp17"'
    si = html.index(sm)
    ...
    # 4. COMPLEMENTARY -- insert the aula-18 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=17;', 'var totalLessons=18;')
    if html2 != html:
