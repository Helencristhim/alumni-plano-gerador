#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 15 (Writing a Formal Email) nos hubs prof + aluno de
...
Aulas 1-14 ficam intactas. Prof recebe card IN CLASS + stamp + accordion +
...
def patch(path, is_prof):
    ...
    if 'id="ex-lesson-15"' in s or 'data-media="l15-' in s:
        print(f'  SKIP {os.path.basename(path)} (aula 15 ja integrada)')
        return
    orig = len(s)

    # 1. CARD IN CLASS (só prof) — após o </a> da aula 14
    if is_prof:
        anchor = '/professor/walyson-ginaldo-silva-aula14.html?autostart=1'
        i = s.index(anchor)
        close = s.index('</a>', i) + len('</a>')
        s = s[:close] + '\n' + card + s[close:]

    # 2. STAMP — após o stamp14
    m = re.search(r'<div class="stamp" id="stamp14"[^>]*></div>', s)
    assert m, 'stamp14 nao encontrado em ' + path
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 3. ACCORDION Pre-class — após o lesson-card ex-lesson-14 (div balanceada)
    j = s.index('id="ex-lesson-14"')
    card_start = s.rindex('<div', 0, j)
    end = div_end(s, card_start)
    s = s[:end] + '\n' + accordion + s[end:]

    # 3b. COMPLEMENTARES — após o wrapper l14-youtube (div balanceada)
    k = s.index('data-media="l14-youtube"')
    ...
    s, n = re.subn(r'totalLessons=14\b', 'totalLessons=15', s)
    assert n >= 1, 'totalLessons=14 nao encontrado em ' + path
