#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 12 da Gabriela nos hubs (prof + aluno) de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-12 ja existe, aborta.

Ambos os hubs (prof e aluno) ja tem a aula 11 como ultima aula (ex-lesson-11,
stamp11, l11). Por isso AMBOS ancoram em last_n=11. Sem extra_audio/order nesta
aula -> nenhuma entrada [order-l12] no audioMap."""
import os, re
...
def patch(path, is_prof, last_n):
    ...
    # 1. stamp12 apos o ultimo stamp existente
    m = re.search(r'<div class="stamp" id="stamp%d"[^>]*></div>' % last_n, s)
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 2. accordion ex-lesson-12 apos o ultimo ex-lesson existente
    st = s.index('<div class="lesson-card" id="ex-lesson-%d">' % last_n)
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l12 apos o ultimo wrapper l{last_n}
...
patch(os.path.join(ROOT, 'public', 'professor', 'gabriela-paulucci.html'), True, 11)
patch(os.path.join(ROOT, 'public', 'aluno', 'gabriela-paulucci.html'), False, 11)
print('OK')
