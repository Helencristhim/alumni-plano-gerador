...
PROF_OUT = os.path.join(ROOT, 'public', 'professor', 'eduarda-gabriel-aula7.html')
ALUNO_OUT= os.path.join(ROOT, 'public', 'aluno', 'eduarda-gabriel-aula7.html')
...
js, n = re.subn(r'var lessonRanges = \{.*?\};',
                'var lessonRanges = { 7: {start: 1, end: %d} };' % N, js, flags=re.S)
assert n == 1, 'lessonRanges patch'

js, n = re.subn(r'var currentLesson = 1;', 'var currentLesson = 7;', js)
...
# ---- 2b. replace lesson-1 quick-challenge array with aula-7 turn-taking situations ----
CHALLENGES = '''var challenges = [
  { prompt: 'Two people are talking and you have an important point. How do you interrupt politely?', answer: '"Sorry to jump in, but I have a quick point."' },
  { prompt: 'Someone interrupts you before you finish. How do you hold the floor?', answer: '"Hang on, let me finish my point."' },
  { prompt: 'A colleague wants to speak. How do you yield the floor?', answer: '"Please, go ahead."' },
  { prompt: 'You accidentally spoke at the same time as someone. What do you say?', answer: '"Sorry, I did not mean to talk over you."' },
  { prompt: 'You want to return to your earlier point after a tangent.', answer: '"Anyway, as I was saying, the timeline is tight."' },
  { prompt: 'The discussion is going long. How do you suggest finishing?', answer: '"Let us wrap up this topic and move on."' }
];
...
def header(badge, hubhref):
    return ('<body>\n\n'
      ...
      '      <div class="progress-label"><span>Aula 7 &mdash; Turn-Taking: Interrupting &amp; Yielding Politely</span><span id="progressPercent">0%</span></div>\n'
...
WELCOME = '''<div class="exercise-section" style="margin-bottom:1.5rem">
  ...
  <p style="font-size:.88rem;line-height:1.7;margin-bottom:1rem">This week you learn to take your turn in a busy meeting: how to interrupt politely, hold the floor, and yield gracefully using <strong>discourse markers</strong>. Review these emergency turn-taking phrases before class.</p>
  <div style="display:grid;gap:0.8rem">
    ...
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">Hang on, let me finish my point.</strong><button class="audio-btn" onclick="speakText('Hang on, let me finish my point.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">Please, go ahead.</strong><button class="audio-btn" onclick="speakText('Please, go ahead.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">Let&rsquo;s wrap up and move on.</strong><button class="audio-btn" onclick="speakText('Let us wrap up and move on.', this)">Listen</button></div>
  </div>
</div>'''
...
INCLASS_MENU = '''<div class="tab-content" id="tab-inclass">
  <h3 style="font-family:'Cormorant Garamond',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 7</h3>
  <div style="display:flex;flex-direction:column;gap:1rem">
    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" onclick="enterSlideMode(1)" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='rgba(200,200,190,.5)'">
      ...
      <div><div style="font-weight:600;font-size:.95rem">Turn-Taking: Interrupting &amp; Yielding Politely</div><div style="font-size:.8rem;color:var(--text-dim)">Discourse markers &amp; interruption phrases &mdash; ''' + str(N) + ''' slides</div></div>
    </div>
  </div>
</div>'''
...
AUDIT = '''<!--
SCENARIO FIT — Aula 7
Can-do: "I can interrupt politely, hold my turn, and yield the floor in a multi-party meeting using discourse markers."
Gramatica-alvo: discourse markers + interruption/turn-taking phrases (interrupt / hold the floor / yield / return to a point).
Vocab-alvo: jump in, chime in, yield the floor, go ahead, hear out, talk over, wrap up, hang on.
Cenario escolhido: reuniao multi-party do comite de reestruturacao (David=chair, Anna, Eduarda) com tres pessoas disputando a fala.
Por que elicita o alvo: numa reuniao com 3+ falantes o aluno e OBRIGADO a entrar na fala ("Sorry to jump in..."), segurar o turno ("Hang on, let me finish"), ceder ("Please, go ahead") e retomar ("Anyway, as I was saying"). >70% dos itens-alvo sao naturalmente elicitados. Habitat natural do focoLinguistico "discourse markers + interruption phrases".

CONTINUIDADE — Aula 7
Itens novos desta aula: jump in, chime in, yield the floor, go ahead, hear out, talk over, wrap up, hang on; discourse markers (well, anyway, as I was saying) e frases de turn-taking.
Itens revisados (de aulas anteriores): quebrar o gelo + question forms + common ground (Aula 6); cross-border deal / creditors / due diligence / deadline (Aulas 4/5).
Callback no warm-up (slide 2): retoma ATIVAMENTE 2 itens da Aula 6 — o aluno (1) faz uma pergunta social (question form) para quebrar o gelo e (2) menciona "common ground"; so entao faz a ponte para o tema de hoje (a reuniao ja comecou e tres pessoas falam ao mesmo tempo).
NB: nenhum item-alvo novo repete vocabulario ja ensinado. A Aula 2 ensinou "interrupt/clarify/elaborate/agenda/follow up/perspective/proposal/consensus"; a Aula 7 usa vocabulario DISTINTO (jump in, chime in, yield, go ahead, hear out, talk over, wrap up, hang on) e foca em CEDER a vez + discourse markers, nao em re-ensinar "interrupt".
-->
'''

# ---- assemble PROFESSOR ----
prof = (
    head('Professor View &mdash; Eduarda Gabriel | Aula 7 | Turn-Taking | Alumni by Better')
...
aluno = (
    head('Aluno &mdash; Eduarda Gabriel | Aula 7 | Turn-Taking | Alumni by Better')
