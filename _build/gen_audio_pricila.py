#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Gera MP3s ElevenLabs da aula N da Pricila a partir de _build/pricila-aula{N}/phrases.json.
Pula existentes (REGRA C9). Roster REGRA 35 (arthur/ellen/josh/rachel/domi/bella). multilingual_v2.
Uso: ELEVENLABS_API_KEY=... python3 _build/gen_audio_pricila.py N"""
import os, json, time, urllib.request, sys

N = int(sys.argv[1])
HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
OUT  = os.path.join(ROOT, 'public', 'audio', 'pricila-adamo')
VOICES = {'arthur':'sfJopaWaOtauCD3HKX6Q','ellen':'BIvP0GN1cAtSRTxNHnWS','josh':'TxGEqnHWrfWFTfGW9XjX',
...
phrases = json.load(open(os.path.join(HERE, 'pricila-aula%d' % N, 'phrases.json'), encoding='utf-8'))
gen = skip = err = 0
for p in phrases:
...
print('Done aula %d: %d gen, %d skip, %d err' % (N, gen, skip, err))
