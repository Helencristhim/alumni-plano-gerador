#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 14 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-13 intactas. Idempotente: se ex-lesson-14 ja existe, aborta.
Ancoras: stamp13 / ex-lesson-13 / l13-youtube / aula13.html?autostart=1."""
import os, re
...
ORDER_AMAP = '  "[order-l14]": "/audio/erica-maria-machado-santarem/pc14_order_l14.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp14 apos stamp13 — SO se ainda nao existir
    if 'id="stamp14"' not in s:
        m = re.search(r'<div class="stamp" id="stamp13"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp14 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula13.html?autostart=1')
