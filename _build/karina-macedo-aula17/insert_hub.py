#!/usr/bin/env python3
"""Insere a aula 17 nos hubs prof e aluno da Karina -- SO ADITIVO.
...
def patch_hub(path, is_prof):
    s = read(path)
    n0 = len(s)
    assert 'ex-lesson-17' not in s, 'aula 17 JA esta no hub -- abortando (idempotencia)'

    # 1. STAMP -- apos o stamp16
    stamp16 = '<div class="stamp" id="stamp16"'
    end16 = s.index('</div>', s.index(stamp16)) + len('</div>')
    s = s[:end16] + '\n' + stamp + s[end16:]

    # 2. ACCORDION Pre-class -- apos o fechamento do ex-lesson-16
    anchor16 = '<div class="lesson-card" id="ex-lesson-16">'
    p16 = s.index(anchor16)
    nxt = s.find('<div class="lesson-card"', p16 + len(anchor16))
    tab_end = s.find('</div><!-- /tab-exercises -->', p16)
    ...
    # 3. COMPLEMENTARES -- apos o ultimo card l16 (l16-youtube wrapper)
    yt = '<div class="media-card-wrapper" data-media="l16-youtube">'
    pyt = s.index(yt)
    ...
    if comp_tab_end != -1:
        ...
        assert m, 'nao achei fim do wrapper l16-youtube'
        end = pyt + m.end()
        s = s[:end] + '\n' + comp + s[end:]
    ...
    assert 'var totalLessons=16' in s, 'nao achei var totalLessons=16'
    s = s.replace('var totalLessons=16', 'var totalLessons=17')

    # 6 (so prof). IN CLASS card -- apos o card da aula 16
    if is_prof:
        card_anchor = '/professor/karina-macedo-aula16.html?autostart=1'
        pc = s.index(card_anchor)
        a_end = s.index('</a>', pc) + len('</a>')
        s = s[:a_end] + '\n' + card + s[a_end:]

    assert 'ex-lesson-17' in s and 'stamp17' in s and 'l17-series' in s, 'pos-condicao falhou'
    assert 'var totalLessons=17' in s
    if is_prof:
        assert 'karina-macedo-aula17.html?autostart=1' in s
    write(path, s)
