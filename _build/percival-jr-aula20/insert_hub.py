...
amap_lines.append(f'  "[order-l20]": "{AUDIO_BASE}pc20_order_l20.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp20 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp20".*?</div>)', snip, re.S)
stamp20 = ms.group(1)

STAMP19 = ('<div class="stamp" id="stamp19" data-label="Set a Time" '
           "style=\"background-image:url('https://images.unsplash.com/"
           "photo-1501139083538-0139583c060f?w=200&q=80')\"></div>")

AULA19_HREF = 'percival-jr-aula19.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 2. stamp20 (apos stamp19 Set a Time)
    ...
    assert len(re.findall(r'totalLessons=19(?![0-9])', s)) == 1, f'{path}: totalLessons=19 nao unico'
    s = re.sub(r'totalLessons=19(?![0-9])', 'totalLessons=20', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 19
    if is_prof:
        i = s.index(AULA19_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
