#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 20 nos hubs (prof + aluno) de milton-sayegh
de forma ADITIVA. Aulas passadas intactas. Idempotente: se ex-lesson-20 ja existe, SKIP.
Molde: milton-sayegh-aula19/insert_hub.py (sem extra-audio). Ancora no FIM do card
da aula anterior (ex-lesson-19) casando div/div, NUNCA em index('tab-inclass')."""
import os, re
...
def patch(path, is_prof):
    ...
    # 2. accordion ex-lesson-20 apos ex-lesson-19 (casando div/div)
    st = s.index('id="ex-lesson-19">')
    ...
    if is_prof:
        ci = s.index('milton-sayegh-aula19.html?autostart=1')
