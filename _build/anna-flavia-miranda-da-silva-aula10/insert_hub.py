...
ajusta var totalLessons=9 -> 10. Fragmentos vem dos arquivos do builder.
"""
import os
import re
...
CARD10 = (
    '  <a href="/professor/anna-flavia-miranda-da-silva-aula10.html?autostart=1" target="_blank" '
    ...
    'display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem">10</div>\n'
    '    <div><div style="font-weight:600;font-size:.95rem">Review: Professional Communication</div>'
    '<div style="font-size:.8rem;color:var(--text-dim)">Emails, calls, plans &amp; product -- 27 slides</div></div>\n'
    '  </a>')

STAMP9 = ('<div class="stamp" id="stamp9" data-label="Product" style="background-image:url(\''
          'https://images.unsplash.com/photo-1581006852262-e4307cf6283a?w=200&q=80\')"></div>')
STAMP10 = ('\n        <div class="stamp" id="stamp10" data-label="Office" style="background-image:url(\''
           'https://images.unsplash.com/photo-1497215728101-856f4ea42174?w=200&q=80\')"></div>')
...
def patch(path, is_prof):
    ...
    # 2. stamp10
    assert s.count(STAMP9) == 1, f'{path}: ancora stamp9 nao unica'
    s = s.replace(STAMP9, STAMP9 + STAMP10, 1); n += 1
    ...
    # 5. totalLessons
    assert 'var totalLessons=9;' in s, f'{path}: totalLessons=9 nao encontrado'
    ...
    if is_prof:
        ...
        s = s.replace(CARD09_TAIL, CARD09_TAIL + '\n' + CARD10, 1); n += 1
