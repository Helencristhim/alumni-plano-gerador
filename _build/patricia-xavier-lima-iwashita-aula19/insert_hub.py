#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 19 nos hubs (prof + aluno) de patricia-xavier-lima-iwashita
de forma ADITIVA. Aulas 1-18 intactas. Idempotente: se ex-lesson-19 ja existe, aborta.
Ancora na aula 18 (stamp18/ex-lesson-18/l18-youtube/aula18 card). Sem order-audio."""
import os, re
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: stamp18 nao encontrado'
    s = s[:m.end()] + '\n' + stamp + s[m.end():]
    ...
    if is_prof:
        ci = s.index('patricia-xavier-lima-iwashita-aula18.html?autostart=1')
