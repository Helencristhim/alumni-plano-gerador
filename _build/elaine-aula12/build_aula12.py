...
PROF_OUT  = os.path.join(ROOT, 'public', 'professor', 'elaine-mieko-pinho-aula12.html')
ALUNO_OUT = os.path.join(ROOT, 'public', 'aluno', 'elaine-mieko-pinho-aula12.html')
GEN_OUT   = os.path.join(ROOT, 'scripts', 'generate-elaine-aula12-audio.js')
...
AUDIT = '''<!--
SCENARIO FIT — Aula 12
Can-do: "I can order at a coffee shop and make small talk."
Gramatica-alvo: "Can I get / I'll have + drink"; "For here or to go?"; "How's your day going?" (small talk present continuous).
Vocab-alvo: barista, to go, refill, straw, napkin, muffin, decaf, mug.
Cenario escolhido: um coffee shop em New York, pedindo e conversando com a barista.
Por que elicita o alvo: pedir um cafe e esperar OBRIGA o aluno a usar "Can I get", responder "for here or to go" e puxar small talk. >70% dos itens-alvo elicitados.

CONTINUIDADE — Aula 12
Itens novos: barista, to go, refill, straw, napkin, muffin, decaf, mug; "Can I get...?", "For here or to go?", "I'll have...", "How's your day going?".
Itens revisados (L9/L11): order, "Could I have"; "How much is it?", pay.
Callback no warm-up (slide 2): retoma ATIVAMENTE 2 itens da Aula 11 — o aluno ouve e repete "How much is it?" e "Can I pay by card?", e faz a ponte: no cafe voce tambem pede e paga, mas e mais rapido e amigavel.
-->
'''

def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 12 | Coffee Shops and Cafes')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 12</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Coffee Shops and Cafes</div><div style="font-size:.8rem;color:var(--text-dim)">Ordering &amp; small talk -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
