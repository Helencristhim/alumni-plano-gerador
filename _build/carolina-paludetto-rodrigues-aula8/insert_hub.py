...
ajusta var totalLessons=8. Os fragmentos vem dos arquivos do builder.
"""
import os
import re
...
STAMP7_READER = ('<div class="stamp" id="stamp7" data-label="Reader" '
                 'style="background-image:url(\'https://images.unsplash.com/'
                 'photo-1512820790803-83ca734da794?w=200&q=80\')"></div>')
STAMP8 = ('\n        <div class="stamp" id="stamp8" data-label="Foodie" '
          'style="background-image:url(\'https://images.unsplash.com/'
          'photo-1504674900247-0877df9cc836?w=200&q=80\')"></div>')

CARD07_TAIL = (
    '      <div><div style="font-weight:600;font-size:.95rem">Reading Adventures</div>'
    '<div style="font-size:.8rem;color:var(--text-dim)">Past Simple (Regular Verbs) -- 27 slides</div></div>\n'
    '    </a>')
...
def patch(path, is_prof):
    ...
    # 2. stamp8
    assert s.count(STAMP7_READER) == 1, f'{path}: ancora stamp7 Reader nao unica'
    s = s.replace(STAMP7_READER, STAMP7_READER + STAMP8, 1); n += 1
    ...
    # 5. totalLessons
    assert 'var totalLessons=7;' in s
    ...
    if is_prof:
        ...
        new_tail = CARD07_TAIL + '\n' + card
        s = s.replace(CARD07_TAIL, new_tail, 1); n += 1
