#!/usr/bin/env python3
"""Inserção ESTRITAMENTE ADITIVA da aula 18 nos hubs prof + aluno da Fabiana.
Âncoras de string inequívocas. Idempotente: aborta se a18 já existe."""
import os
...
def patch(path, is_prof):
    ...
    if 'ex-lesson-18' in html or 'stamp18' in html:
        print(f'  SKIP {os.path.basename(path)} — aula18 já presente'); return
    orig = html

    # STAMP: após o </div> do stamp17
    a = '<div class="stamp" id="stamp17"'
    ...
    # COMPLEMENTARES: após o wrapper l17-youtube
    pos = end_of_wrapper(html, html.index('data-media="l17-youtube"'))
    html = html[:pos] + '\n' + COMP + html[pos:]

    # MENU IN CLASS (só prof): após o </a> do card da aula 17
    if is_prof:
        m = html.index('</a>', html.index('aula17.html?autostart=1')) + len('</a>')
        html = html[:m] + '\n    ' + MENU.strip() + html[m:]
    ...
    # totalLessons 17 -> 18
    html = html.replace('var totalLessons = 17;', 'var totalLessons = 18;')

    assert html != orig and 'ex-lesson-18' in html, 'inserção falhou'
