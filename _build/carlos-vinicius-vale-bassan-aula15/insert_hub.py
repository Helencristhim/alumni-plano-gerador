#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 15 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp15, accordion Pre-class (ex-lesson-15), card IN CLASS (só prof),
bloco Complementares (l15), entradas de audioMap, e totalLessons 14->15.
NÃO toca em nada das aulas 1-14 (inclusive os fixes da Helen). Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula15.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 3 &#8212; Presenting the Business Case: Delivering the Recommendation</div><div style="font-size:.8rem;color:var(--text-dim)">Turn the business case into a short, structured pitch: open with the recommendation, give the rationale, and close with the upshot &#8212; using emphasis to make the case compelling &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-15"' not in s, f'{path}: aula 15 já integrada'
    # 1. stamp15 após stamp14
    stamp14 = ('        <div class="stamp" id="stamp14" data-label="Business Case" '
               "style=\"background-image:url('https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=200&q=80')\"></div>")
    s = sub_once(s, stamp14, stamp14 + '\n' + STAMP15, 'stamp')
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=14;', '  var totalLessons=15;', 'totalLessons')
    # 6. audioMap entries
