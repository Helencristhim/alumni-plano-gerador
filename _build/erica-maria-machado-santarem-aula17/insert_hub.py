#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-16 intactas. Idempotente: se ex-lesson-17 ja existe, aborta.
Ancoras: stamp16/ex-lesson-16/l16-youtube/aula16.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp17" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l17]": "/audio/erica-maria-machado-santarem/pc17_order_l17.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp17 apos stamp16 — SO se ainda nao existir
    if 'id="stamp17"' not in s:
        m = re.search(r'<div class="stamp" id="stamp16"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp17 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula16.html?autostart=1')
