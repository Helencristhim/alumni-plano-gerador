#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 16 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-16 ja existe, aborta.
Molde: eduarda-gabriel-aula15/insert_hub.py. Ancora de complementares = l15-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l16]": "/audio/eduarda-gabriel/order_l16_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-15">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l16 apos o ultimo wrapper l15 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula15.html?autostart=1')
