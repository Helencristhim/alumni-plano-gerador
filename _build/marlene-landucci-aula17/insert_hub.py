#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insert builder-produced hub_snippets.html (aula 17) into Marlene's existing hubs.
ADDITIVE ONLY: never touches lessons 1-16. Idempotent (aborts if stamp17 already present)."""
import os
import re
...
def insert_common(c):
    assert 'id="stamp17"' not in c, 'stamp17 já presente — abortando (não duplicar)'
    # (b) stamp after stamp16
    m = re.search(r'(<div class="stamp" id="stamp16"[^>]*></div>)', c)
    c = c[:m.end()] + '\n        ' + STAMP + c[m.end():]
    # (c) ex-lesson-17 accordion right before the Celebration card
    anchor = '\n<!-- Celebration card -->'
    if anchor in c:
        i = c.index(anchor)
    else:
    ...
    c = c[:m.end()] + AUDIO + '\n' + c[m.end():]
    # totalLessons 16 -> 17
    c = re.sub(r'var totalLessons=16', 'var totalLessons=17', c)
    c = re.sub(r'var totalLessons = 16', 'var totalLessons = 17', c)
    return c
...
c = insert_common(c)
# (a) menu card: after the aula-16 menu <a>...</a> card.
mark = 'marlene-landucci-aula16.html?autostart=1'
mi = c.index(mark)
...
# ---- ALUNO ---- (no tab-inclass menu card; TOTAL_AULAS 16->17)
ap = os.path.join(ROOT, 'public', 'aluno', 'marlene-landucci.html')
c = open(ap, encoding='utf-8').read()
c = insert_common(c)
c = re.sub(r"window\.TOTAL_AULAS=16\b", 'window.TOTAL_AULAS=17', c)
