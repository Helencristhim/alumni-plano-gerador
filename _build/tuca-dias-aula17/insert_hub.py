...
SNIP = open('_build/tuca-dias-aula17/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/tuca-dias-aula16.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('tuca-dias-aula17.html?autostart') == 0, 'aula17 card ja existe'
    ...
    # --- 3. ACCORDION Pre-class (after ex-lesson-16, before next major block) ---
    ...
    ci = h.index('data-media="l16-youtube"')
