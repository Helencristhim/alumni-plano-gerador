#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 19 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula18.

Aulas 1-18 intactas. Idempotente: se ex-lesson-19 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp19, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l19]) e ajusta totalLessons=19.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l19]": "/audio/carolina-paludetto-rodrigues/pc19_order_sequence.mp3",'
...
assert 'id="stamp19"' in stamp, stamp[:60]
assert 'id="ex-lesson-19"' in accordion, accordion[:60]
assert 'data-media="l19-youtube"' in comp, comp[:60]
assert amap_entries and 'a19_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp18 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]

    # 2. accordion ex-lesson-19 apos ex-lesson-18
    st = s.index('<div class="lesson-card" id="ex-lesson-18">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l19 apos o ultimo wrapper de l18
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula18.html?autostart=1')
