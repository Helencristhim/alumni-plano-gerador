#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 20 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-20 ja existe, aborta.
Molde: eduarda-gabriel-aula19/insert_hub.py. Ancora de complementares = l19-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l20]": "/audio/eduarda-gabriel/order_l20_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-19">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l20 apos o ultimo wrapper l19 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula19.html?autostart=1')
