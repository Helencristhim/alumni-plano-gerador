#!/usr/bin/env python3
# Insere os snippets da aula 13 nos hubs prof + aluno do roberto-rezende (SO ADITIVO).
# Legado: hub aluno tem buracos (ex-lesson 7-9, stamps 7-11 faltando), PRE-EXISTENTE.
# NAO consertamos: so anexamos a aula 13 apos o ULTIMO stamp/ex-lesson presente em CADA arquivo.
import re

SNIP = open('_build/roberto-rezende-aula13/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    # --- 1. MENU CARD (prof only) — apos o card da aula 12 ---
    if is_prof:
        anchor = '/professor/roberto-rezende-aula12.html?autostart=1'
        assert h.count('roberto-rezende-aula13.html?autostart') == 0, 'aula13 card ja existe'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        h = h[:close] + '\n            ' + menu_card.strip() + h[close:]

    # --- 2. STAMP (apos o ULTIMO stamp presente; difere prof vs aluno) ---
    assert 'id="stamp13"' not in h, 'stamp13 ja existe'
    last_stamp = [m.start() for m in re.finditer(r'id="stamp\d+"', h)][-1]
    sclose = h.index('</div>', last_stamp) + len('</div>')
    h = h[:sclose] + '\n        ' + stamp.strip() + h[sclose:]

    # --- 3. ACCORDION Pre-class (apos ex-lesson-12, antes do proximo bloco de aba) ---
    assert 'id="ex-lesson-13"' not in h, 'ex-lesson-13 ja existe'
    li = h.index('id="ex-lesson-12"')
    if is_prof:
    ...
    # --- 4. COMPLEMENTARY (apos o ULTIMO wrapper l12, dentro de tab-complementary) ---
    assert 'data-media="l13-' not in h, 'l13 complementary ja existe'
    last_l12 = [m.start() for m in re.finditer(r'data-media="l12-', h)][-1]
    wrap_end = end_of_wrapper(h, last_l12)
    h = h[:wrap_end] + '\n' + comp + '\n' + h[wrap_end:]
    ...
    # --- 6. totalLessons 12 -> 13 (ADITIVO) ---
    h2 = re.sub(r'var totalLessons ?= ?12', 'var totalLessons=13', h)
