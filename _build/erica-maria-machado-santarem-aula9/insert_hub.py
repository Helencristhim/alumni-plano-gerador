#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 9 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-8 intactas. Idempotente: se ex-lesson-9 ja existe, aborta.
Molde aula8: ancoras stamp8/ex-lesson-8/l8-youtube/aula8.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp9" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l9]": "/audio/erica-maria-machado-santarem/pc9_order_l9.mp3",'
...
def patch(path, is_prof):
    ...
    if 'id="ex-lesson-9"' in s:
        print(f'  SKIP {os.path.basename(path)} (aula 9 ja integrada)')
        return
    orig_len = len(s)

    # 1. stamp9 apos stamp8 — SO se ainda nao existir
    if 'id="stamp9"' not in s:
        m = re.search(r'<div class="stamp" id="stamp8"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp9 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')

    # 2. accordion ex-lesson-9 apos ex-lesson-8
    st = s.index('<div class="lesson-card" id="ex-lesson-8">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l9 apos o ultimo wrapper l8
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula8.html?autostart=1')
        ce = s.index('</a>', ci) + len('</a>')
        s = s[:ce] + '\n' + card_prof + s[ce:]

    # 5. entradas de audioMap (logo apos a abertura do objeto) + [order-l9]
