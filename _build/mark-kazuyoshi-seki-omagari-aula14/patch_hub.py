#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Patch aditivo do hub (prof + aluno) da Aula 14 do Mark. NAO toca aulas 1-13."""
import os
import re
...
# audioMap entries (entre <script> e </script> da secao 4) + [order-l14] (extra_audio)
m = re.search(r'<!-- 4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
entries = m.group(1).rstrip('\n')
entries += '\n  "[order-l14]": "/audio/mark-kazuyoshi-seki-omagari/pc14_order_passive.mp3",'

# stamp14
stamp = re.search(r'(<div class="stamp" id="stamp14".*?</div>)', snip).group(1)

STAMP13 = '''<div class="stamp" id="stamp13" data-label="What If" style="background-image:url('https://images.unsplash.com/photo-1499209974431-9dddcece7f88?w=200&q=80')"></div>'''
...
CARD = ('    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" '
        'onclick="window.location.href=\'/professor/mark-kazuyoshi-seki-omagari-aula14.html?autostart=1\'" '
        ...
        '      <div><div style="font-weight:600;font-size:.95rem">Make It Happen</div><div style="font-size:.8rem;color:var(--text-dim)">Phase 2 &mdash; the passive voice in everyday processes and instructions &mdash; 28 slides</div></div>\n'
        '    </div>')
CARD_ANCHOR = ('<div><div style="font-weight:600;font-size:.95rem">What Could Have Happened</div><div style="font-size:.8rem;color:var(--text-dim)">Phase 2 &mdash; modal perfects (should have, could have, might have) for past speculation &mdash; 28 slides</div></div>\n    </div>')
...
def patch(path, is_prof):
    ...
    # 2. stamp14
    assert c.count(STAMP13) == 1, f'{path}: stamp13 anchor'
    c = c.replace(STAMP13, STAMP13 + '\n' + stamp, 1)
    ...
    # 5. totalLessons
    c = re.sub(r'var totalLessons\s*=\s*13;', 'var totalLessons=14;', c, count=1)
