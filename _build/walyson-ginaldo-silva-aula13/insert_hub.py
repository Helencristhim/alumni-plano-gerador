#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 13 (Negotiating a Deal) nos hubs prof + aluno de
...
Aulas 1-12 ficam intactas. Prof recebe card IN CLASS + stamp + accordion +
...
def patch(path, is_prof):
    ...
    if 'id="ex-lesson-13"' in s or 'data-media="l13-' in s:
        print(f'  SKIP {os.path.basename(path)} (aula 13 ja integrada)')
        return
    orig = len(s)

    # 1. CARD IN CLASS (só prof) — após o </a> da aula 12
    if is_prof:
        anchor = '/professor/walyson-ginaldo-silva-aula12.html?autostart=1'
        i = s.index(anchor)
        close = s.index('</a>', i) + len('</a>')
        s = s[:close] + '\n' + card + s[close:]

    # 2. STAMP — após o stamp12
    m = re.search(r'<div class="stamp" id="stamp12"[^>]*></div>', s)
    assert m, 'stamp12 nao encontrado em ' + path
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 3. ACCORDION Pre-class — após o lesson-card ex-lesson-12 (div balanceada)
    j = s.index('id="ex-lesson-12"')
    card_start = s.rindex('<div', 0, j)
    end = div_end(s, card_start)
    s = s[:end] + '\n' + accordion + s[end:]

    # 3b. COMPLEMENTARES — após o wrapper l12-youtube (div balanceada)
    k = s.index('data-media="l12-youtube"')
    ...
    s, n = re.subn(r'totalLessons=12\b', 'totalLessons=13', s)
    assert n >= 1, 'totalLessons=12 nao encontrado em ' + path
