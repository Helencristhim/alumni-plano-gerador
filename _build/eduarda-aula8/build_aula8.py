...
PROF_OUT = os.path.join(ROOT, 'public', 'professor', 'eduarda-gabriel-aula8.html')
ALUNO_OUT= os.path.join(ROOT, 'public', 'aluno', 'eduarda-gabriel-aula8.html')
...
js, n = re.subn(r'var lessonRanges = \{.*?\};',
                'var lessonRanges = { 8: {start: 1, end: %d} };' % N, js, flags=re.S)
assert n == 1, 'lessonRanges patch'

js, n = re.subn(r'var currentLesson = 1;', 'var currentLesson = 8;', js)
...
# ---- 2b. replace lesson-1 quick-challenge array with aula-8 defending-a-position situations ----
CHALLENGES = '''var challenges = [
  { prompt: 'You want to introduce your opinion in a meeting. What do you say?', answer: '"In my view, we should act now."' },
  { prompt: 'Someone challenges your claim. How do you support it?', answer: '"The data suggests the market will recover. I can back it up with numbers."' },
  { prompt: 'Argue a consequence using the first conditional.', answer: '"If we wait, we will lose the deal."' },
  { prompt: 'A partner asks about the risk. How do you address the downside?', answer: '"The downside is limited: if the market falls, our capital is protected."' },
  { prompt: 'You disagree with an estimate. How do you push back politely?', answer: '"I want to push back on that estimate. In my view, it is too low."' },
  { prompt: 'Summarize your position in one line.', answer: '"The bottom line is: if we act now, we will lead the deal."' }
];
...
def header(badge, hubhref):
    return ('<body>\n\n'
      ...
      '      <div class="progress-label"><span>Aula 8 &mdash; Defending a Position: Arguing with Data</span><span id="progressPercent">0%</span></div>\n'
...
WELCOME = '''<div class="exercise-section" style="margin-bottom:1.5rem">
  ...
  <p style="font-size:.88rem;line-height:1.7;margin-bottom:1rem">This week you learn to defend a position with data: state your opinion, back it up with evidence, and argue consequences using the <strong>first conditional</strong>. Review these emergency phrases before class.</p>
  <div style="display:grid;gap:0.8rem">
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">In my view, we should act now.</strong><button class="audio-btn" onclick="speakText('In my view, we should act now.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">The data suggests the market will recover.</strong><button class="audio-btn" onclick="speakText('The data suggests the market will recover.', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">What is the downside?</strong><button class="audio-btn" onclick="speakText('What is the downside?', this)">Listen</button></div>
    <div style="background:var(--bg-elevated);padding:0.8rem 1rem;border-radius:8px;display:flex;align-items:center;justify-content:space-between;gap:0.8rem;flex-wrap:wrap;"><strong style="font-size:0.9rem">If we wait, we will lose the deal.</strong><button class="audio-btn" onclick="speakText('If we wait, we will lose the deal.', this)">Listen</button></div>
  </div>
</div>'''
...
INCLASS_MENU = '''<div class="tab-content" id="tab-inclass">
  <h3 style="font-family:'Cormorant Garamond',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 8</h3>
  <div style="display:flex;flex-direction:column;gap:1rem">
    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" onclick="enterSlideMode(1)" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='rgba(200,200,190,.5)'">
      ...
      <div><div style="font-weight:600;font-size:.95rem">Defending a Position: Arguing with Data</div><div style="font-size:.8rem;color:var(--text-dim)">First conditional &amp; opinion frames &mdash; ''' + str(N) + ''' slides</div></div>
    </div>
  </div>
</div>'''
...
AUDIT = '''<!--
SCENARIO FIT — Aula 8
Can-do: "I can defend a position with an opinion, evidence, and first-conditional consequences in a committee."
Gramatica-alvo: first conditional (If + present, ... will + base) + opinion frames (In my view / The data suggests / The bottom line is).
Vocab-alvo: in my view, the data suggests, make a case, back up, upside, downside, bottom line, push back.
Cenario escolhido: investment committee — Eduarda defende uma recomendacao e um socio cetico (Daniel) faz push back.
Por que elicita o alvo: num comite de investimento o aluno e OBRIGADO a dar opiniao ("In my view..."), embasar com dados ("The data suggests..."), argumentar consequencia ("If we wait, we will lose the deal") e tratar upside/downside. >70% dos itens-alvo sao naturalmente elicitados. Habitat natural do focoLinguistico "first conditional + opinion frames".

CONTINUIDADE — Aula 8
Itens novos desta aula: in my view, the data suggests, make a case, back up, upside, downside, bottom line, push back; first conditional.
Itens revisados (de aulas anteriores): turn-taking da Aula 7 (jump in / go ahead); deal / creditors / capital / market (Aulas 4/5); numbers (Aula 3).
Callback no warm-up (slide 2): retoma ATIVAMENTE 2 itens da Aula 7 — o aluno (1) entra na fala educadamente ("Sorry to jump in") e (2) cede a vez ("Please, go ahead"); so entao faz a ponte para o tema de hoje (agora que tem o turno, precisa DEFENDER a posicao com dados).
NB: vocabulario novo distinto de tudo ja ensinado. A Aula 2 ensinou perspective/proposal/consensus; a Aula 8 usa opinion frames DIFERENTES (in my view, the data suggests, bottom line) + first conditional, sem re-ensinar nada.
-->
'''

# ---- assemble PROFESSOR ----
prof = (
    head('Professor View &mdash; Eduarda Gabriel | Aula 8 | Defending a Position | Alumni by Better')
...
aluno = (
    head('Aluno &mdash; Eduarda Gabriel | Aula 8 | Defending a Position | Alumni by Better')
