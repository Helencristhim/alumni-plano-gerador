#!/usr/bin/env python3
# Insere os snippets da aula 13 nos hubs prof + aluno do Victor (SO ADITIVO).
import re

SNIP = open('_build/victor-malvezi-paschotto-aula13/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/victor-malvezi-paschotto-aula12.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('victor-malvezi-paschotto-aula13.html?autostart') == 0, 'aula13 card ja existe'
        h = h[:close] + '\n            ' + menu_card.strip() + h[close:]

    # --- 2. STAMP (after stamp12) ---
    si = h.index('id="stamp12"')
    sclose = h.index('</div>', si) + len('</div>')
    assert 'id="stamp13"' not in h, 'stamp13 ja existe'
    h = h[:sclose] + '\n        ' + stamp.strip() + h[sclose:]

    # --- 3. ACCORDION Pre-class (after ex-lesson-12) ---
    ...
    ci = h.index('data-media="l12-youtube"')
    ...
    h2 = h.replace('var totalLessons=12;', 'var totalLessons=13;')
...
patch('public/professor/victor-malvezi-paschotto.html', True)
patch('public/aluno/victor-malvezi-paschotto.html', False)
