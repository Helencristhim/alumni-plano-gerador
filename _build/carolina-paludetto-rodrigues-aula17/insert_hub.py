#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 17 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula16.

Aulas 1-16 intactas. Idempotente: se ex-lesson-17 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp17, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l17]) e ajusta totalLessons=17.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l17]": "/audio/carolina-paludetto-rodrigues/pc17_order_sequence.mp3",'
...
assert 'id="stamp17"' in stamp, stamp[:60]
assert 'id="ex-lesson-17"' in accordion, accordion[:60]
assert 'data-media="l17-youtube"' in comp, comp[:60]
assert amap_entries and 'a17_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp16 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]

    # 2. accordion ex-lesson-17 apos ex-lesson-16
    st = s.index('<div class="lesson-card" id="ex-lesson-16">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l17 apos o ultimo wrapper de l16
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula16.html?autostart=1')
        ce = s.index('</a>', ci) + len('</a>')
        s = s[:ce] + '\n' + card_prof + s[ce:]

    # 5. entradas de audioMap (logo apos a abertura do objeto) + [order-l17]
