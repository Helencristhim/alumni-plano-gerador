#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 14 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-14 ja existe, aborta.
Molde: eduarda-gabriel-aula13/insert_hub.py. Ancora de complementares = l13-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l14]": "/audio/eduarda-gabriel/order_l14_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-13">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l14 apos o ultimo wrapper l13 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula13.html?autostart=1')
