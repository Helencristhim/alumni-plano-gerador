#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Patch aditivo do hub (prof + aluno) da Aula 9 do Mark. NAO toca aulas 1-8."""
import os
import re
...
# audioMap entries (entre <script> e </script> da secao 4) + [order-l9] (extra_audio)
m = re.search(r'<!-- 4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
entries = m.group(1).rstrip('\n')
entries += '\n  "[order-l9]": "/audio/mark-kazuyoshi-seki-omagari/pc9_order_sequence.mp3",'

# stamp9
stamp = re.search(r'(<div class="stamp" id="stamp9".*?</div>)', snip).group(1)

STAMP8 = '''<div class="stamp" id="stamp8" data-label="Services" style="background-image:url('https://images.unsplash.com/photo-1581092160562-40aa08e78837?w=200&q=80')"></div>'''
...
CARD = ('    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" '
        'onclick="window.location.href=\'/professor/mark-kazuyoshi-seki-omagari-aula9.html?autostart=1\'" '
        ...
        '      <div><div style="font-weight:600;font-size:.95rem">Bring It All Together</div><div style="font-size:.8rem;color:var(--text-dim)">Mini-presentation &mdash; signposting &amp; delivery &mdash; 28 slides</div></div>\n'
        '    </div>')
CARD_ANCHOR = ('Causative have/get &mdash; describing services &mdash; 28 slides</div></div>\n    </div>\n  </div>')
...
def patch(path, is_prof):
    ...
    # 2. stamp9
    assert c.count(STAMP8) == 1, f'{path}: stamp8 anchor'
    c = c.replace(STAMP8, STAMP8 + '\n' + stamp, 1)
    ...
    # 5. totalLessons
    c = re.sub(r'var totalLessons\s*=\s*8;', 'var totalLessons=9;', c, count=1)
    # 6. IN CLASS card (prof only)
    if is_prof:
        assert c.count(CARD_ANCHOR) == 1, f'{path}: card anchor'
        c = c.replace(CARD_ANCHOR, 'Causative have/get &mdash; describing services &mdash; 28 slides</div></div>\n    </div>\n' + CARD + '\n  </div>', 1)
    assert c != orig
