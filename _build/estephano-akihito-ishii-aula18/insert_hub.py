#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 18 nos hubs (prof + aluno) de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-18 ja existe, aborta."""
import os, re
...
ORDER_AMAP = '  "[order-l18]": "/audio/estephano-akihito-ishii/a18_order_how_the_app_works.mp3",'
...
def patch(path, is_prof):
    ...
    # 2. accordion ex-lesson-18 apos o FIM do card ex-lesson-17 (casando div/div)
    st = s.index('<div class="lesson-card" id="ex-lesson-17">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l17 apos o ultimo wrapper l17
    ...
    if is_prof:
        ci = s.index('estephano-akihito-ishii-aula17.html?autostart=1')
