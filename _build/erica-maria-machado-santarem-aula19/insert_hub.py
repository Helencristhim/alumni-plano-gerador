#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 19 nos hubs (prof + aluno) de erica-maria-machado-santarem
de forma ADITIVA. Aulas 1-18 intactas. Idempotente: se ex-lesson-19 ja existe, aborta.
Ancoras: stamp18/ex-lesson-18/l18-youtube/aula18.html?autostart=1.
A insercao de stamp e GUARDADA (so insere se id="stamp19" ainda nao existir)."""
import os, re
...
ORDER_AMAP = '  "[order-l19]": "/audio/erica-maria-machado-santarem/pc19_order_l19.mp3",'
...
def patch(path, is_prof):
    ...
    # 1. stamp19 apos stamp18 — SO se ainda nao existir
    if 'id="stamp19"' not in s:
        m = re.search(r'<div class="stamp" id="stamp18"[^>]*></div>', s)
        s = s[:m.end()] + '\n' + stamp + s[m.end():]
    else:
        print(f'  (stamp19 ja existe em {os.path.basename(path)} — insercao de stamp pulada)')
    ...
    if is_prof:
        ci = s.index('erica-maria-machado-santarem-aula18.html?autostart=1')
