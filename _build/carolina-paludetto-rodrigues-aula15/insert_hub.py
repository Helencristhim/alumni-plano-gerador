#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 15 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula14.

Aulas 1-14 intactas. Idempotente: se ex-lesson-15 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp15, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l15]) e ajusta totalLessons=15.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l15]": "/audio/carolina-paludetto-rodrigues/pc15_order_sequence.mp3",'
...
assert 'id="stamp15"' in stamp, stamp[:60]
assert 'id="ex-lesson-15"' in accordion, accordion[:60]
assert 'data-media="l15-youtube"' in comp, comp[:60]
assert amap_entries and 'a15_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp14 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]

    # 2. accordion ex-lesson-15 apos ex-lesson-14
    st = s.index('<div class="lesson-card" id="ex-lesson-14">')
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l15 apos o ultimo wrapper de l14
    yi = s.index('data-media="l14-youtube"')
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula14.html?autostart=1')
