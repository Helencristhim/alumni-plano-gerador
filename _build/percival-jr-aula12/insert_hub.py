...
amap_lines.append(f'  "[order-l12]": "{AUDIO_BASE}pc12_order_l12.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp12 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp12".*?</div>)', snip, re.S)
stamp12 = ms.group(1)

STAMP11 = ('<div class="stamp" id="stamp11" data-label="Meetings" '
           "style=\"background-image:url('https://images.unsplash.com/"
           "photo-1517048676732-d65bc937f952?w=200&q=80')\"></div>")

AULA11_HREF = 'percival-jr-aula11.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 2. stamp12 (apos stamp11 Meetings)
    ...
    assert len(re.findall(r'totalLessons=11(?![0-9])', s)) == 1, f'{path}: totalLessons=11 nao unico'
    s = re.sub(r'totalLessons=11(?![0-9])', 'totalLessons=12', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 11
    if is_prof:
        i = s.index(AULA11_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
