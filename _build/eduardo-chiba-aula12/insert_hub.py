#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 12 nos hubs (prof + aluno) de eduardo-chiba de forma
ADITIVA. Hub legado da Helen: ex-lesson 1-11 + aulas 5-11 como cards <a>
standalone (SO no prof). Aulas 1-11 ficam intactas. Idempotente.
...
def make_card(s):
    """Clona o card <a> da aula 11 (estilo legado da Helen, so no prof) e reescreve p/ aula 12."""
    m = re.search(r'<a href="/professor/eduardo-chiba-aula11\.html".*?</a>', s, flags=re.S)
    if not m:
        raise RuntimeError('card da aula 11 nao encontrado no prof')
    card = m.group(0)
    card = card.replace('eduardo-chiba-aula11.html', 'eduardo-chiba-aula12.html')
    card = card.replace('>11<', '>12<')
    card = card.replace('The Opening Act -- Structuring Presentations',
                        'Data Storytelling -- Making Numbers Alive')
    card = card.replace('Signposting language -- 27 slides',
                        'Comparatives &amp; trends language -- 27 slides')
    return card
...
def patch(path, is_prof):
    ...
    # 2. accordion ex-lesson-12 apos o fim do ex-lesson-11
    st = s.index('<div class="lesson-card" id="ex-lesson-11">')
    en = div_end(s, st)
    s = s[:en] + '\n\n' + accordion + s[en:]

    # 3. complementares l12 apos o ultimo wrapper l11 (l11-youtube)
    yi = s.index('data-media="l11-youtube"')
    wst = s.rfind('<div class="media-card-wrapper"', 0, yi)
    wen = div_end(s, wst)
    s = s[:wen] + '\n' + comp + s[wen:]

    # 4. card de lancamento da aula 12 (SO no prof; aluno nao tem cards IN CLASS)
    if is_prof:
        card = make_card(s)
        a11 = list(re.finditer(r'<a href="/professor/eduardo-chiba-aula11\.html".*?</a>', s, flags=re.S))
        target = a11[-1]
        s = s[:target.end()] + '\n\n' + card + s[target.end():]
