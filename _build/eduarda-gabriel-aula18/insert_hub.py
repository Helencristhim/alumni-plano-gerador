#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 18 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-18 ja existe, aborta.
Molde: eduarda-gabriel-aula17/insert_hub.py. Ancora de complementares = l17-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l18]": "/audio/eduarda-gabriel/order_l18_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-17">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l18 apos o ultimo wrapper l17 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula17.html?autostart=1')
