#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 nos hubs (prof + aluno) de victor-malvezi-paschotto
de forma ADITIVA. Prof tem cards IN CLASS (<a> ?autostart=1); aluno NAO tem cards
IN CLASS (so stamp + accordion + complementares + audioMap). Aulas 1-16 intactas.
Idempotente."""
import os
import re
...
def patch(path, is_prof):
    ...
    # 2. accordion ex-lesson-17 apos o fim do ex-lesson-16
    st = s.index('<div class="lesson-card" id="ex-lesson-16">')
    en = div_end(s, st)
    s = s[:en] + '\n\n' + accordion + s[en:]

    # 3. complementares l17 apos o ultimo wrapper l16 (l16-youtube)
    yi = s.index('data-media="l16-youtube"')
    wst = s.rfind('<div class="media-card-wrapper"', 0, yi)
    wen = div_end(s, wst)
    s = s[:wen] + '\n' + comp + s[wen:]

    # 4. card de lancamento IN CLASS da aula 17 (SO no prof; aluno nao tem cards IN CLASS)
    if is_prof:
        a16 = list(re.finditer(r'<a href="/professor/victor-malvezi-paschotto-aula16\.html\?autostart=1".*?</a>', s, flags=re.S))
        if not a16:
            raise RuntimeError('card IN CLASS da aula 16 nao encontrado no prof')
        target = a16[-1]
        s = s[:target.end()] + '\n' + card + s[target.end():]
