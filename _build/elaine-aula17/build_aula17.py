#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build standalone Aula 17 (Elaine) — Sightseeing: Tickets and Tours. Splices authored
...
a aula ENTREGUE e a INLINE no monolitico (ver inline_17.py)."""
import os, re, json
...
PROF_OUT  = os.path.join(REFDIR, 'elaine-mieko-pinho-aula17.html')
ALUNO_OUT = os.path.join(REFDIR, 'elaine-mieko-pinho-aula17-aluno.html')
...
def build(scaffold, badge_is_aluno):
    h = scaffold
    h = must(h, 'Aula 9 | Restaurant Basics', 'Aula 17 | Sightseeing: Tickets and Tours')
    ...
    if not badge_is_aluno:
        MENU = ('  <h3 style="font-family:\'Cormorant Garamond\',serif;font-size:1.3rem;margin-bottom:1rem">IN CLASS &mdash; Aula 17</h3>\n'
                ...
                '      <div><div style="font-weight:600;font-size:.95rem">Sightseeing -- Tickets and Tours</div><div style="font-size:.8rem;color:var(--text-dim)">Booking a tour -- 28 slides</div></div>\n'
                '    </div>\n  </div>\n')
...
print('OK aula17 standalone (ref) built; audioMap keys', len(phrases))
