#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 14 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp14, accordion Pre-class (ex-lesson-14), card IN CLASS (só prof),
bloco Complementares (l14), entradas de audioMap, e totalLessons 13->14.
NÃO toca em nada das aulas 1-13 (inclusive os fixes da Helen). Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula14.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 3 &#8212; Making the Business Case: Justifying the Investment</div><div style="font-size:.8rem;color:var(--text-dim)">Make the business case for the next phase: compare the options (build vs buy, this vendor vs that), and justify which one delivers the most value &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-14"' not in s, f'{path}: aula 14 já integrada'
    # 1. stamp14 após stamp13
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=13;', '  var totalLessons=14;', 'totalLessons')
    # 6. audioMap entries
