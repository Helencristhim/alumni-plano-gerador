#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere ADITIVAMENTE os snippets da aula 20 (gerados pelo build_from_model.py)
nos hubs (prof + aluno) da Carolina. Molde carolina-aula19.

Aulas 1-19 intactas. Idempotente: se ex-lesson-20 ja existe, aborta.
So adiciona: card IN CLASS (so prof), stamp20, accordion Pre-class, bloco de
Complementares, entradas de audioMap (+ [order-l20]) e ajusta totalLessons=20.
"""
import os
import re
...
ORDER_AMAP = '  "[order-l20]": "/audio/carolina-paludetto-rodrigues/pc20_order_sequence.mp3",'
...
assert 'id="stamp20"' in stamp, stamp[:60]
assert 'id="ex-lesson-20"' in accordion, accordion[:60]
assert 'data-media="l20-youtube"' in comp, comp[:60]
assert amap_entries and 'a20_' in amap_entries
...
def patch(path, is_prof):
    ...
    assert m, f'{path}: ancora stamp19 nao encontrada'
    s = s[:m.end()] + '\n        ' + stamp + s[m.end():]
    ...
    # 3. complementares l20 apos o ultimo wrapper de l19
    ...
    if is_prof:
        ci = s.index('carolina-paludetto-rodrigues-aula19.html?autostart=1')
