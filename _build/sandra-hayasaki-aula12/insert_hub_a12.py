#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 12 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp12, (3) ex-lesson-12 accordion,
(4) complementary aula 12 block, (5) audioMap entries; bumps totalLessons 11->12.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-12"' in html and 'id="stamp12"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 12 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula11 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula11.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp11 div (within stamps-row)
    sm = 'id="stamp11"'
    si = html.index(sm)
    ...
    # 3. ACCORDION -- insert after ex-lesson-11 closes, i.e. right before '</div><!-- /tab-exercises -->'
    ...
    # 4. COMPLEMENTARY -- insert the aula-12 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=11;', 'var totalLessons=12;')
    if html2 != html:
