#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 15 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-15 ja existe, aborta.
Molde: eduarda-gabriel-aula14/insert_hub.py. Ancora de complementares = l14-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l15]": "/audio/eduarda-gabriel/order_l15_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-14">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l15 apos o ultimo wrapper l14 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula14.html?autostart=1')
