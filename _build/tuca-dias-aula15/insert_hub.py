#!/usr/bin/env python3
# Insere os snippets da aula 15 nos hubs prof + aluno (SO ADITIVO).
import re, sys

SNIP = open('_build/tuca-dias-aula15/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/tuca-dias-aula14.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('tuca-dias-aula15.html?autostart') == 0, 'aula15 card ja existe'
        h = h[:close] + '\n            ' + menu_card.strip() + h[close:]

    # --- 2. STAMP (after stamp14) ---
    si = h.index('id="stamp14"')
    sclose = h.index('</div>', si) + len('</div>')
    assert 'id="stamp15"' not in h, 'stamp15 ja existe'
    h = h[:sclose] + '\n        ' + stamp.strip() + h[sclose:]

    # --- 3. ACCORDION Pre-class (after ex-lesson-14, before next major block) ---
    ...
    ci = h.index('data-media="l14-youtube"')
    ...
    h2 = h.replace('var totalLessons=14;', 'var totalLessons=15;')
