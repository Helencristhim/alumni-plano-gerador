#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 11 nos hubs prof e aluno de helio-santana.
Extrai as 5 seções do hub_snippets.html e as encaixa por âncora de string.
NÃO toca nas aulas 1-10 — só APPEND."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula10_link = '/professor/helio-santana-aula10.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula10_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula10 não encontrado ({kind})'
        a10_block = m.group(1)
        assert t.count(a10_block) == 1, f'card aula10 ambíguo ({kind})'
        t = t.replace(a10_block, a10_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp10
    stamp10 = '<div class="stamp" id="stamp10" data-label="Planes de Futuro" style="background-image:url(\'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=200&q=80\')"></div>'
    assert stamp10 in t, f'âncora stamp10 não encontrada ({kind})'
    assert t.count(stamp10) == 1, f'stamp10 ambíguo ({kind})'
    t = t.replace(stamp10, stamp10 + '\n' + stamp, 1)

    # 3. ACCORDION -> antes do fechamento da tab-exercises (após o ex-lesson-10)
    ...
    # 6. totalLessons 10 -> 11 (TOTAL_AULAS fica 50)
