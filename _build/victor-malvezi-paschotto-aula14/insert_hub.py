#!/usr/bin/env python3
# Insere os snippets da aula 14 nos hubs prof + aluno do Victor (SO ADITIVO).
import re

SNIP = open('_build/victor-malvezi-paschotto-aula14/hub_snippets.html', encoding='utf-8').read()
...
def patch(path, is_prof):
    ...
    if is_prof:
        anchor = '/professor/victor-malvezi-paschotto-aula13.html?autostart=1'
        i = h.index(anchor)
        close = h.index('</a>', i) + len('</a>')
        assert h.count('victor-malvezi-paschotto-aula14.html?autostart') == 0, 'aula14 card ja existe'
    ...
    # --- 3. ACCORDION Pre-class (after ex-lesson-13) ---
