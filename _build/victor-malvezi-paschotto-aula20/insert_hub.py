#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 20 nos hubs (prof + aluno) de victor-malvezi-paschotto
de forma ADITIVA. Prof tem cards IN CLASS (<a> ?autostart=1); aluno NAO tem cards
IN CLASS (so stamp + accordion + complementares + audioMap). Aulas 1-19 intactas.
Idempotente."""
import os
import re
...
def patch(path, is_prof):
    ...
    # 3. complementares l20 apos o ultimo wrapper l19 (l19-youtube)
    yi = s.index('data-media="l19-youtube"')
    wst = s.rfind('<div class="media-card-wrapper"', 0, yi)
    wen = div_end(s, wst)
    s = s[:wen] + '\n' + comp + s[wen:]

    # 4. card de lancamento IN CLASS da aula 20 (SO no prof; aluno nao tem cards IN CLASS)
    if is_prof:
        a19 = list(re.finditer(r'<a href="/professor/victor-malvezi-paschotto-aula19\.html\?autostart=1".*?</a>', s, flags=re.S))
        if not a19:
            raise RuntimeError('card IN CLASS da aula 19 nao encontrado no prof')
        target = a19[-1]
        s = s[:target.end()] + '\n' + card + s[target.end():]
    ...
    # 6. totalLessons -> 20 (TOTAL_AULAS fica 24, intacto)
