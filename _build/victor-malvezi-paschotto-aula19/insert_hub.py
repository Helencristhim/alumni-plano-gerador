#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 19 nos hubs (prof + aluno) de victor-malvezi-paschotto
de forma ADITIVA. Prof tem cards IN CLASS (<a> ?autostart=1); aluno NAO tem cards
IN CLASS (so stamp + accordion + complementares + audioMap). Aulas 1-18 intactas.
Idempotente."""
import os
import re
...
def patch(path, is_prof):
    ...
    # 3. complementares l19 apos o ultimo wrapper l18 (l18-youtube)
    yi = s.index('data-media="l18-youtube"')
    wst = s.rfind('<div class="media-card-wrapper"', 0, yi)
    wen = div_end(s, wst)
    s = s[:wen] + '\n' + comp + s[wen:]

    # 4. card de lancamento IN CLASS da aula 19 (SO no prof; aluno nao tem cards IN CLASS)
    if is_prof:
        a18 = list(re.finditer(r'<a href="/professor/victor-malvezi-paschotto-aula18\.html\?autostart=1".*?</a>', s, flags=re.S))
        if not a18:
            raise RuntimeError('card IN CLASS da aula 18 nao encontrado no prof')
        target = a18[-1]
        s = s[:target.end()] + '\n' + card + s[target.end():]
    ...
    # 6. totalLessons -> 19 (TOTAL_AULAS fica 24, intacto)
