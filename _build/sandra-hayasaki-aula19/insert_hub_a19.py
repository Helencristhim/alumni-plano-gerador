#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 19 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp19, (3) ex-lesson-19 accordion,
(4) complementary aula 19 block, (5) audioMap entries; bumps totalLessons 18->19.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-19"' in html and 'id="stamp19"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 19 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula18 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula18.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp18 div (within stamps-row)
    sm = 'id="stamp18"'
    si = html.index(sm)
    ...
    # 4. COMPLEMENTARY -- insert the aula-19 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=18;', 'var totalLessons=19;')
    if html2 != html:
