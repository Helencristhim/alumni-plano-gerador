#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Patch aditivo do hub (prof + aluno) da Aula 8 do Mark. NAO toca aulas 1-7."""
import os
import re
...
# audioMap entries (entre <script> e </script> da secao 4) + [order-l8] (extra_audio)
m = re.search(r'<!-- 4\. ENTRADAS.*?<script>\n(.*?)</script>', snip, re.S)
entries = m.group(1).rstrip('\n')
entries += '\n  "[order-l8]": "/audio/mark-kazuyoshi-seki-omagari/pc8_order_sequence.mp3",'

# stamp8
stamp = re.search(r'(<div class="stamp" id="stamp8".*?</div>)', snip).group(1)

STAMP7 = '''<div class="stamp" id="stamp7" data-label="Connectors" style="background-image:url('https://images.unsplash.com/photo-1505373877841-8d25f7d46678?w=200&q=80')"></div>'''
...
CARD = ('    <div style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s" '
        'onclick="window.location.href=\'/professor/mark-kazuyoshi-seki-omagari-aula8.html?autostart=1\'" '
        ...
        '      <div><div style="font-weight:600;font-size:.95rem">Getting Things Done</div><div style="font-size:.8rem;color:var(--text-dim)">Causative have/get &mdash; describing services &mdash; 28 slides</div></div>\n'
        '    </div>')
CARD_ANCHOR = ('Discourse markers for structured opinions &mdash; 28 slides</div></div>\n    </div>\n  </div>')
...
def patch(path, is_prof):
    ...
    # 2. stamp8
    assert c.count(STAMP7) == 1, f'{path}: stamp7 anchor'
    c = c.replace(STAMP7, STAMP7 + '\n' + stamp, 1)
    ...
    # 5. totalLessons
    c = re.sub(r'var totalLessons\s*=\s*7;', 'var totalLessons=8;', c, count=1)
    # 6. IN CLASS card (prof only)
    if is_prof:
        assert c.count(CARD_ANCHOR) == 1, f'{path}: card anchor'
        c = c.replace(CARD_ANCHOR, 'Discourse markers for structured opinions &mdash; 28 slides</div></div>\n    </div>\n' + CARD + '\n  </div>', 1)
    assert c != orig
