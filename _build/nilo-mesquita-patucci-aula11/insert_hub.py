#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 11 nos hubs prof+aluno do Nilo (aditivo, idempotente).

Replica EXATAMENTE o padrão da aula 10 (PR #313): card + accordion no fim da
tab-exercises, complementares no fim da tab-complementary, audioMap mesclado,
stamp11 após stamp10, totalLessons 10 -> 11. Não toca aulas 1-10."""
import os, re
...
STAMP11 = '\n        <div class="stamp" id="stamp11" data-label="Meetings" style="background-image:url(\'https://images.unsplash.com/photo-1517048676732-d65bc937f952?w=200&q=80\')"></div>'

def card_for(view):
    href = f'/{view}/nilo-mesquita-patucci-aula11.html?autostart=1'
    return (f'<a href="{href}" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit;margin-top:1rem" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
            f'  <div style="width:48px;height:48px;background:var(--accent);border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem">11</div>\n'
            f'  <div><div style="font-weight:600;font-size:.95rem">Meeting Dynamics</div><div style="font-size:.8rem;color:var(--text-dim)">Making suggestions -- leading and joining meetings -- 28 slides</div></div>\n'
            f'</a>\n')
...
def patch(view):
    ...
    if 'id="ex-lesson-11"' in s:
        print(f'  {view}: já tem aula 11 — pulando'); return
    # 1. stamp 11 após stamp 10
    assert STAMP10 in s, f'{view}: stamp10 não encontrado'
    s = s.replace(STAMP10, STAMP10 + STAMP11, 1)
    ...
    print(f'  {view}: inserido (stamp11, accordion ex-lesson-11, card, complementares l11, {len(amap_lines)} audioMap, totalLessons=11)')
