#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 19 da Gabriela nos hubs (prof + aluno) de forma ADITIVA.
Aulas passadas intactas (incluindo o gap legado 6-10 do hub aluno). Idempotente:
se ex-lesson-19 ja existe, aborta.

Ambos os hubs (prof e aluno) ja tem a aula 18 como ultima aula (ex-lesson-18,
stamp18, l18). Por isso AMBOS ancoram em last_n=18."""
import os, re
...
def patch(path, is_prof, last_n):
    ...
    # 1. stamp19 apos o ultimo stamp existente
    m = re.search(r'<div class="stamp" id="stamp%d"[^>]*></div>' % last_n, s)
    s = s[:m.end()] + '\n' + stamp + s[m.end():]

    # 2. accordion ex-lesson-19 apos o ultimo ex-lesson existente
    st = s.index('<div class="lesson-card" id="ex-lesson-%d">' % last_n)
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l19 apos o ultimo wrapper l{last_n}
...
patch(os.path.join(ROOT, 'public', 'professor', 'gabriela-paulucci.html'), True, 18)
patch(os.path.join(ROOT, 'public', 'aluno', 'gabriela-paulucci.html'), False, 18)
print('OK')
