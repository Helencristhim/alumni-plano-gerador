# -*- coding: utf-8 -*-
"""Insere os snippets da aula 17 nos hubs PROF e ALUNO da Simone (aditivo).
Adiciona o stamp17 (ainda nao existe no hub) e ajusta totalLessons para 17."""
import os, re
...
audio_lines += f'\n  "[order-l17]": "{AUDIO_BASE}a17_order_sequence.mp3",'

# menu card (snippet section 1)
mc = re.search(r'1\. CARD.*?-->\n(.*?)\n\n<!-- 2\.', snip, re.S)
menu_card = mc.group(1).rstrip('\n')

# stamp17 (snippet section 2)
ms = re.search(r'2\. STAMP.*?-->\n(.*?)\n\n<!-- 3\.', snip, re.S)
stamp17 = ms.group(1).rstrip('\n')
assert 'id="stamp17"' in stamp17, 'stamp17 ausente no snippet (config tem stamp id 17?)'

def patch(path, is_prof):
    s = open(path, encoding='utf-8').read()
    orig_len = len(s)
    assert 'id="ex-lesson-17"' not in s, f'{path}: aula 17 ja inserida'
    assert 'id="stamp17"' not in s, f'{path}: stamp17 ja existe'
    assert 'id="ex-lesson-16"' in s, f'{path}: aula 16 ausente (hub desatualizado?)'
    ...
    # 5) stamp17 na stamps-row (apos stamp16)
    m16 = re.search(r'(<div class="stamp" id="stamp16"[^>]*></div>)', s)
    assert m16, f'{path}: stamp16 ausente na stamps-row'
    s = s.replace(m16.group(1), m16.group(1) + '\n' + stamp17, 1)

    # 6) totalLessons -> 17 (NAO mexer em TOTAL_AULAS=48, que e o total do programa)
    s = re.sub(r'totalLessons\s*=\s*\d+', 'totalLessons = 17', s)
