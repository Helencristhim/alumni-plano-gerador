#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insert builder-produced hub_snippets.html (aula 20) into Marlene's existing hubs.
ADDITIVE ONLY: never touches lessons 1-19. Idempotent (aborts if stamp20 already present)."""
import os
import re
...
def insert_common(c):
    assert 'id="stamp20"' not in c, 'stamp20 já presente — abortando (não duplicar)'
    # (b) stamp after stamp19
    m = re.search(r'(<div class="stamp" id="stamp19"[^>]*></div>)', c)
    c = c[:m.end()] + '\n        ' + STAMP + c[m.end():]
    # (c) ex-lesson-20 accordion right before the Celebration card
    anchor = '\n<!-- Celebration card -->'
    if anchor in c:
        i = c.index(anchor)
    else:
    ...
    c = c[:m.end()] + AUDIO + '\n' + c[m.end():]
    # totalLessons 19 -> 20
    c = re.sub(r'var totalLessons=19', 'var totalLessons=20', c)
    c = re.sub(r'var totalLessons = 19', 'var totalLessons = 20', c)
    return c
...
c = insert_common(c)
# (a) menu card: after the aula-19 menu <a>...</a> card.
mark = 'marlene-landucci-aula19.html?autostart=1'
mi = c.index(mark)
