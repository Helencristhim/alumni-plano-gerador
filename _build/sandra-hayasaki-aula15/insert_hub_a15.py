#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Idempotent ADDITIVE inserter of aula 15 hub snippets into Sandra's hubs.
Inserts: (1) IN CLASS menu card [prof only], (2) stamp15, (3) ex-lesson-15 accordion,
(4) complementary aula 15 block, (5) audioMap entries; bumps totalLessons 14->15.
...
def process(path, is_prof):
    ...
    # idempotency guard
    if 'id="ex-lesson-15"' in html and 'id="stamp15"' in html:
        print(f"  [{os.path.basename(path)}] already has aula 15 -> skip")
        return

    # 1. IN CLASS menu card (prof only) -- insert after aula14 <a>...</a>
    if is_prof:
        anchor = '/professor/sandra-hayasaki-aula14.html?autostart=1'
        i = html.index(anchor)
        close = html.index('</a>', i) + len('</a>')
        html = html[:close] + '\n' + CARD + html[close:]
        changes.append('inclass-card')

    # 2. STAMP -- insert right after stamp14 div (within stamps-row)
    sm = 'id="stamp14"'
    si = html.index(sm)
    ...
    # 4. COMPLEMENTARY -- insert the aula-15 media block right before the close marker
    ...
    html2 = html.replace('var totalLessons=14;', 'var totalLessons=15;')
    if html2 != html:
