#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insercao ADITIVA da aula 19 nos hubs prof+aluno (passo 7 do pipeline do modelo).
NAO reconstroi o hub: so insere stamp19, accordion ex-lesson-19, card IN CLASS,
bloco de complementares l19- e entradas de audioMap (pc19_ + [order-l19]); seta totalLessons=19.
Aulas 1..18 ficam intactas.
"""
import os, re, sys
...
# audioMap: so as entradas pc19_ (frases do hub/preclass) + [order-l19]
pc_lines = [ln for ln in snippet.splitlines() if 'pc19_' in ln and ln.strip().startswith('"')]
order_line = '  "[order-l19]": "/audio/%s/a19_order_pitch.mp3",' % SLUG
audio_block = '\n'.join([order_line] + [ln.rstrip() for ln in pc_lines])

STAMP = ('<div class="stamp" id="stamp19" data-label="Sales Pitch" '
         'style="background-image:url(\'https://images.unsplash.com/photo-1551434678-e076c223a692?w=200&q=80\')"></div>')

def card(view):
    return (
        '<a href="/%s/%s-aula19.html?autostart=1"  style="display:flex;align-items:center;gap:1rem;padding:1.2rem;'
        ...
        'align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem">19</div>\n'
        '      <div><div style="font-weight:600;font-size:.95rem">The Sales Pitch -- Persuasive Language &amp; Benefits</div>'
        '<div style="font-size:.8rem;color:var(--text-dim)">Block 4 -- Pitching &amp; Persuasion -- 27 slides</div></div>\n'
        '    </a>' % (view, SLUG))

def patch(path, view):
    s = open(path, encoding='utf-8').read()
    assert 'id="stamp19"' not in s, 'aula 19 ja inserida em ' + path
    # 1. stamp19 apos stamp18
    stamp18 = ('<div class="stamp" id="stamp18" data-label="Meeting Sim" '
               'style="background-image:url(\'https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=200&q=80\')"></div>')
    assert stamp18 in s, 'stamp18 nao encontrado em ' + path
    s = s.replace(stamp18, stamp18 + '\n' + STAMP, 1)
    # 2. accordion Pre-class antes de <!-- /tab-exercises -->
    s = s.replace('<!-- /tab-exercises -->', preclass + '\n<!-- /tab-exercises -->', 1)
    # 3. card IN CLASS apos o card da aula18 (so o hub do professor tem aba IN CLASS;
    #    o aluno tem 2 abas, sem IN CLASS — REGRA 3)
    aula18_card_end = ('<div style="font-size:.8rem;color:var(--text-dim)">Block 3 Review &amp; Full Meeting Simulation -- 27 slides</div></div>\n'
                       '    </a>')
    if aula18_card_end in s:
        s = s.replace(aula18_card_end, aula18_card_end + '\n    ' + card(view), 1)
