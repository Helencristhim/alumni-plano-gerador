#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insert builder-produced hub_snippets.html (aula 16) into Marlene's existing hubs.
ADDITIVE ONLY: never touches lessons 1-15. Idempotent (aborts if stamp16 already present)."""
import os
import re
...
def insert_common(c):
    assert 'id="stamp16"' not in c, 'stamp16 já presente — abortando (não duplicar)'
    # (b) stamp after stamp15
    m = re.search(r'(<div class="stamp" id="stamp15"[^>]*></div>)', c)
    c = c[:m.end()] + '\n        ' + STAMP + c[m.end():]
    # (c) ex-lesson-16 accordion right before the Celebration card
    anchor = '\n<!-- Celebration card -->'
    if anchor in c:
        i = c.index(anchor)
    else:
    ...
    c = c[:m.end()] + AUDIO + '\n' + c[m.end():]
    # totalLessons 15 -> 16
    c = re.sub(r'var totalLessons=15', 'var totalLessons=16', c)
    c = re.sub(r'var totalLessons = 15', 'var totalLessons = 16', c)
    return c
...
c = insert_common(c)
# (a) menu card: after the aula-15 menu <a>...</a> card.
mark = 'marlene-landucci-aula15.html?autostart=1'
mi = c.index(mark)
...
# ---- ALUNO ---- (no tab-inclass menu card; TOTAL_AULAS 15->16)
ap = os.path.join(ROOT, 'public', 'aluno', 'marlene-landucci.html')
c = open(ap, encoding='utf-8').read()
c = insert_common(c)
c = re.sub(r"window\.TOTAL_AULAS=15\b", 'window.TOTAL_AULAS=16', c)
