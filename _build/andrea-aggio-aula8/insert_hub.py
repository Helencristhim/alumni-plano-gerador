#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula N no hub existente da Andrea Aggio (prof + aluno), aditivo.
O card da aula N entra logo apos o card da aula N-1. Stamp N entra apos stamp{N-1}."""
import os
...
def process(path, href_prefix):
    ...
    # 1. IN CLASS card: insert after the previous aula card
    ...
    # 2. Pre-class accordion: before </div><!-- /tab-exercises -->
    marker = '</div><!-- /tab-exercises -->'
    im = s.index(marker)
    s = s[:im] + accordion + '\n\n' + s[im:]

    # 3. Complementary block: before </div><!-- /tab-complementary -->
    cmarker = '</div><!-- /tab-complementary -->'
    ic = s.index(cmarker)
    s = s[:ic] + comp + '\n\n' + s[ic:]

    # 4. audioMap entries: after 'var audioMap = {'
    ...
    # 6. STAMP: insert stampN right after stamp{PREV}
