#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 15 nos hubs prof e aluno de daniel-bastos."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula14_link = '/professor/daniel-bastos-aula14.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula14_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula14 não encontrado ({kind})'
        a14_block = m.group(1)
        assert t.count(a14_block) == 1
        t = t.replace(a14_block, a14_block + '\n' + menu_card, 1)

    stamp14 = '<div class="stamp" id="stamp14" data-label="Resolver Problemas" style="background-image:url(\'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=200&q=80\')"></div>'
    t = insert_after(t, stamp14, '\n' + stamp, f'{kind} stamp')
    ...
    t2 = t.replace('var totalLessons=14;', 'var totalLessons=15;')
