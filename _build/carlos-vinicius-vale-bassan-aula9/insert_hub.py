#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Inserção ADITIVA da aula 9 do Carlos nos hubs (prof + aluno).
Só ADICIONA: stamp9, accordion Pre-class (ex-lesson-9), card IN CLASS (só prof),
bloco Complementares (l9), entradas de audioMap, e totalLessons 8->9.
NÃO toca em nada das aulas 1-8. Cada âncora é validada (exatamente 1 ocorrência)."""
import os, re, sys
...
STAMP9 = ('        <div class="stamp" id="stamp9" data-label="Negotiation" '
          "style=\"background-image:url('https://images.unsplash.com/photo-1521791136064-7986c2920216?w=200&q=80')\"></div>")

INCLASS_CARD = (
    '<a href="/professor/' + SLUG + '-aula9.html#slides" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor=\'var(--accent)\'" onmouseout="this.style.borderColor=\'rgba(200,200,190,.5)\'">\n'
    ...
    '  <div><div style="font-weight:600;font-size:.95rem">Block 2 &#8212; Negotiation in Practice</div><div style="font-size:.8rem;color:var(--text-dim)">Bargaining and Conditional Offers &#8212; 27 slides</div></div>\n'
    '</a>\n')
...
def integrate(path, is_prof):
    s = read(path)
    assert 'id="ex-lesson-9"' not in s, f'{path}: aula 9 já integrada'
    # 1. stamp9 após stamp8
    stamp8 = ('        <div class="stamp" id="stamp8" data-label="Block 1" '
              "style=\"background-image:url('https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=200&q=80')\"></div>")
    s = sub_once(s, stamp8, stamp8 + '\n' + STAMP9, 'stamp')
    ...
    # 5. totalLessons
    s = sub_once(s, '  var totalLessons=8;', '  var totalLessons=9;', 'totalLessons')
    # 6. audioMap entries
