#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 19 nos hubs (prof + aluno) de Eduarda de forma ADITIVA.
Aulas passadas intactas. Idempotente: se ex-lesson-19 ja existe, aborta.
Molde: eduarda-gabriel-aula18/insert_hub.py. Ancora de complementares = l18-youtube."""
import os, re
...
ORDER_AMAP = '  "[order-l19]": "/audio/eduarda-gabriel/order_l19_sequence.mp3",'
...
def patch(path, is_prof):
    ...
    st = re.search(r'<div class="lesson-card[^"]*" id="ex-lesson-18">', s).start()
    en = div_end(s, st)
    s = s[:en] + '\n' + accordion + s[en:]

    # 3. complementares l19 apos o ultimo wrapper l18 (3a midia = youtube)
    ...
    if is_prof:
        ci = s.index('eduarda-gabriel-aula18.html?autostart=1')
