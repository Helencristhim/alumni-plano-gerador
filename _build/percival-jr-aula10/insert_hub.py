...
amap_lines.append(f'  "[order-l10]": "{AUDIO_BASE}pc10_order_l10.mp3",')
amap_block = '\n'.join(amap_lines)
...
# --- stamp10 (parte 2) ---
ms = re.search(r'2\. STAMP.*?\n(<div class="stamp" id="stamp10".*?</div>)', snip, re.S)
stamp10 = ms.group(1)

STAMP9 = ('<div class="stamp" id="stamp9" data-label="Compare" '
          "style=\"background-image:url('https://images.unsplash.com/"
          "photo-1450101499163-c8848c66ca85?w=200&q=80')\"></div>")

AULA9_HREF = 'percival-jr-aula9.html?autostart=1'
...
def patch(path, is_prof):
    ...
    # 5. totalLessons 9 -> 10
    assert len(re.findall(r'totalLessons=9(?![0-9])', s)) == 1, f'{path}: totalLessons=9 nao unico'
    s = re.sub(r'totalLessons=9(?![0-9])', 'totalLessons=10', s, count=1); n += 1

    # 6. card IN CLASS (so prof): inserir apos o </a> que fecha o card da aula 9
    if is_prof:
        i = s.index(AULA9_HREF)
        j = s.index('</a>', i) + len('</a>')
        s = s[:j] + '\n' + card + s[j:]; n += 1
