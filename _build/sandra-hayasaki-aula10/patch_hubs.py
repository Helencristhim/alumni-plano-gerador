#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Insere os snippets da aula 10 (Sandra) nos hubs prof e aluno (aditivo)."""
import os
...
amap_lines.append('  "[order-l10]": "/audio/sandra-hayasaki/a10_order_restaurant.mp3",')
amap_block = '\n'.join(amap_lines)

STAMP10 = ('        <div class="stamp" id="stamp10" data-label="Restaurant" '
           "style=\"background-image:url('https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=200&q=80')\"></div>")
...
CARD = '''    <a href="/professor/sandra-hayasaki-aula10.html?autostart=1" style="display:flex;align-items:center;gap:1rem;padding:1.2rem;background:rgba(255,255,255,.5);backdrop-filter:blur(8px);border:1px solid rgba(200,200,190,.5);border-radius:10px;cursor:pointer;transition:all .3s;text-decoration:none;color:inherit" onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='rgba(200,200,190,.5)'">
      <div style="width:48px;height:48px;background:var(--accent-light);border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:1.1rem">10</div>
      <div><div style="font-weight:600;font-size:.95rem">At the Restaurant</div><div style="font-size:.8rem;color:var(--text-dim)">Would like + ordering food -- 27 slides</div></div>
    </a>'''

ANCHOR_AMAP = '  "[order-l9]": "/audio/sandra-hayasaki/a9_order_shopping.mp3",\n};'
ANCHOR_STAMP9 = ('        <div class="stamp" id="stamp9" data-label="Shopping" '
...
def patch(path, is_prof):
    ...
    s = s.replace(ANCHOR_AMAP,
                  '  "[order-l9]": "/audio/sandra-hayasaki/a9_order_shopping.mp3",\n' + amap_block + '\n};', 1)
    # 2. stamp10
    assert ANCHOR_STAMP9 in s, f'{path}: anchor stamp9'
    s = s.replace(ANCHOR_STAMP9, ANCHOR_STAMP9 + '\n' + STAMP10, 1)
    # 3. ex-lesson-10 accordion (antes do fechamento da tab-exercises)
    ...
    # 5. totalLessons -> 10
    assert 'var totalLessons=9;' in s, f'{path}: totalLessons'
    s = s.replace('var totalLessons=9;', 'var totalLessons=10;', 1)
    # 6. IN CLASS card (so prof tem a tab)
    if is_prof:
        anchor_card = ('      <div><div style="font-weight:600;font-size:.95rem">At the Supermarket</div>'
                       '<div style="font-size:.8rem;color:var(--text-dim)">How much / how many + quantities -- 27 slides</div></div>\n'
                       '    </a>\n  </div>')
