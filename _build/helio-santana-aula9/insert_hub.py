#!/usr/bin/env python3
"""Inserção SÓ ADITIVA da aula 9 nos hubs prof e aluno de helio-santana.
Extrai as 5 seções do hub_snippets.html e as encaixa por âncora de string.
NÃO toca nas aulas 1-8 — só APPEND."""
import re, os, sys
...
for kind, path in KINDS.items():
    ...
    if kind == 'professor':
        aula8_link = '/professor/helio-santana-aula8.html?autostart=1'
        m = re.search(r'(<a href="' + re.escape(aula8_link) + r'".*?</a>)', t, re.S)
        assert m, f'card aula8 não encontrado ({kind})'
        a8_block = m.group(1)
        assert t.count(a8_block) == 1, f'card aula8 ambíguo ({kind})'
        t = t.replace(a8_block, a8_block + '\n' + menu_card, 1)

    # 2. STAMP -> após stamp8
    stamp8 = '<div class="stamp" id="stamp8" data-label="El Proceso Antifraude" style="background-image:url(\'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=200&q=80\')"></div>'
    assert stamp8 in t, f'âncora stamp8 não encontrada ({kind})'
    assert t.count(stamp8) == 1, f'stamp8 ambíguo ({kind})'
    t = t.replace(stamp8, stamp8 + '\n' + stamp, 1)

    # 3. ACCORDION -> antes do fechamento da tab-exercises (após o ex-lesson-8)
    ...
    # 6. totalLessons 8 -> 9 (TOTAL_AULAS fica 50)
    t2 = t.replace('var totalLessons=8', 'var totalLessons=9')
