...
> Drive ID: `1Sd2Ljdlh2tiEzfHBPUH1q3VBcTFmkSo3`
...
## Adendo Normativo 03 — Apoio funcional, integridade pedagógica e safeguards de produção

Private Class · Alumni by Better · Vigência: 24/08/2026

## 1. Status normativo, escopo e precedência

Este adendo tem caráter normativo e complementa os Documentos pedagógicos 00–06 e a Série P. Ele regula regimes distintos de apoio funcional em português para A0/Pre-A1 e A1/A2 nas atividades autônomas, o modo de consulta do Pre-class na visão do professor e safeguards adicionais de integridade pedagógica, produção, interface e QA.
...
- garantir apoio funcional em português no Pre-class e no Post-class segundo regimes próprios para A0/Pre-A1 e A1/A2, sem substituir o contato necessário com o inglês;
...
| Apoio recolhível em português | Visível em A0/Pre-A1 e A1/A2, conforme o regime aplicável | Ausente |
...
- [ ] Os cards de checkpoint apresentam os comandos Antes da aula e Após a aula com intervalos distintos e corretos: antes, somente as aulas do bloco já realizadas; após, todas as aulas do bloco, incluindo o checkpoint recém-realizado. "Estado pedagógico do ciclo" abre o painel existente sem duplicar seu conteúdo.
- [ ] Rótulos do Teacher's Guide que introduzem conteúdo usam dois-pontos em todas as ocorrências.

## 28. Card de checkpoint — comandos antes e após a aula

No comando "Antes da aula", o intervalo deve incluir somente as aulas do bloco já realizadas, excluindo a aula de checkpoint que ainda será ministrada.

No comando "Após a aula", o intervalo deve incluir todas as aulas do bloco, incluindo a aula de checkpoint recém-realizada.

Exemplo: em um bloco formado pelas aulas 21–24, o comando "Antes da aula" deve orientar a consulta aos registros das aulas 21–23; o comando "Após a aula" deve orientar a consolidação dos registros das aulas 21–24.

A diferença entre os intervalos é intencional e não constitui inconsistência: antes da aula, não se deve tratar como existente o registro de uma aula ainda não realizada. Em ambos os comandos, "Estado pedagógico do ciclo" deve funcionar como botão ou link para o painel já existente na visão docente, preservando o retorno ao card e sem duplicar seu conteúdo.

### 28.1 Fonte única em superfícies multilíngues

Quando a mesma informação tiver apresentações legítimas em idiomas ou interfaces diferentes, fonte única significa **um único registro responsável, não uma única string literal**. As superfícies vinculadas podem variar conforme idioma e função, mas não podem divergir em ação, intervalo, momento ou consequência nem ser redigitadas em objetos independentes.

Nos cards de checkpoint, a superfície em português e a orientação correspondente em inglês derivam do mesmo registro: Antes da aula inclui somente as aulas do bloco já realizadas; Após a aula inclui o bloco completo, incluindo a aula de checkpoint recém-realizada.

### 28.2 Proporcionalidade do apoio em português em A1 e A2

A cobertura não é comprovada apenas por contagem mínima. Em A1, o apoio é mais frequente, alcança instruções menos transparentes e atividades de maior carga e exige revisão do percurso completo para assegurar autonomia assíncrona. Em A2, é seletivo, concentra-se em maior carga e evita repetição quando a operação já estiver estabelecida, mantendo contato predominante com o inglês.

Em ambos os níveis, o apoio começa recolhido, cobre a instrução necessária, não traduz integralmente o insumo, opções, respostas ou linguagem-alvo, não antecipa rationale ou Answer Key e não aparece como interação discente na visão do professor. Os mínimos funcionam como piso, não como aprovação automática.
...
# PARTE G — Regime específico de apoio funcional para A0/Pre-A1

*Atualização normativa · 03 de setembro de 2026*

As restrições desta parte A para A1/A2 permanecem vigentes como regime próprio. Elas **não são copiadas automaticamente para A0/Pre-A1**, cuja necessidade de acesso operacional é maior e deve ser calibrada pela competência-alvo.

No Pre-class e no Post-class A0/Pre-A1, o inglês original permanece disponível. Quando a compreensão daquele inglês não for a competência-alvo, o apoio em português pode abranger todo o conteúdo necessário para compreender e executar a tarefa, incluindo instruções, enunciados, frases, alternativas e opções.

- Usar formulação portuguesa curta, concreta e literal.
- Não revelar resposta, eliminar contraste, traduzir o áudio/transcript como atalho ou resolver antecipadamente a operação cognitiva.
- Quando o objetivo for compreender precisamente o próprio texto, áudio, expressão, vocabulário-alvo ou alternativa linguística, limitar o apoio para preservar a evidência.
- Manter o componente exclusivamente na visão do aluno e inicialmente recolhido.

### G.1 Apoio no In-class A0/Pre-A1

O apoio escrito em português pode aparecer seletivamente e de forma visualmente secundária quando necessário para orientar a tarefa. Mediação oral, modelagem, repetição e apoio visual reduzem a necessidade de tradução ampla, mas não a proíbem. Não presumir que todos os alunos Pre-A1 necessitem a mesma quantidade de português.

### G.2 QA específico

- Nenhum Pre/Post-class Pre-A1 fica inacessível por falta de apoio necessário à execução.
- A tradução não substitui a evidência que a atividade pretende obter.
- Ausência de descritor CEFR não é confundida com ausência de necessidade pedagógica.
- O regime A1/A2 continua válido e não é aplicado automaticamente a A0/Pre-A1.

# PARTE H — Controles, persistência, reset e integridade de estado

*Atualização normativa · 04 de setembro de 2026*

## 36. Check e Redo em atividades verificáveis

Em atividades nas quais o acionamento seguinte ao Check reinicia a tentativa, o rótulo visível e o nome acessível do controle devem representar a ação efetivamente disponível em cada estado:

- antes da verificação, o controle apresenta **Check**;
- depois da verificação, o mesmo controle passa a apresentar **Redo**;
- Redo reinicia somente a atividade correspondente, limpa respostas, seleções e estados de correção, oculta a devolutiva anterior e devolve o controle ao estado Check;
- as demais atividades, fases da aula e aulas do ciclo permanecem intactas.

Não é permitido manter o rótulo Check quando o acionamento seguinte executará um reset. Texto visível, nome acessível, estado funcional e ação por mouse ou teclado devem permanecer equivalentes.

Quando o estado da atividade for persistido, a relação entre tentativa, verificação e rótulo deve permanecer coerente depois da recarga. Uma atividade recarregada como verificada apresenta Redo; uma atividade reiniciada apresenta Check e não recupera correção ou devolutiva anterior.

## 37. Reset lesson

Reset lesson reinicia integralmente o estado do In-class da aula ativa. Não se limita ao slide ou à atividade visível e não alcança dados fora desse escopo.

A confirmação deve informar claramente que serão **apagados**:

- respostas, seleções, campos e estados de correção do In-class da aula ativa;
- feedbacks revelados e progresso dos slides dessa aula;
- o status de aula finalizada e todas as fontes derivadas desse status.

A mesma confirmação deve informar que serão **preservados**:

- Pre-class e Post-class;
- registro pós-aula e Estado pedagógico do ciclo;
- outras aulas e quaisquer dados fora do In-class da aula ativa.

Após a confirmação, a aula retorna ao primeiro slide, o status de conclusão é removido e o novo estado é persistido. Respostas, correções ou conclusão não podem reaparecer depois da recarga. Cancelar a confirmação não produz alteração parcial.

O estado reiniciado deve derivar de uma fonte canônica e ser isolado por aluno, aula, atividade, superfície e papel. Não implementar o reset por listas independentes de chaves que possam deixar resíduos ou apagar Pre-class, Post-class, registro pós-aula, Estado pedagógico do ciclo, outra aula ou dados de outro papel.

## 38. Validação bloqueante dos controles

- [ ] Check muda para Redo somente depois da verificação e ambos os nomes acessíveis correspondem à ação disponível.
- [ ] Redo reinicia apenas a atividade correspondente e devolve seu controle a Check sem alterar outras atividades.
- [ ] A recarga preserva de modo coerente tentativa, verificação, rótulo e devolutiva.
- [ ] Reset lesson exige confirmação clara e o cancelamento preserva integralmente o estado.
- [ ] Reset lesson limpa todo o In-class da aula ativa, retorna ao primeiro slide e remove o status de conclusão e seus derivados.
- [ ] Pre-class, Post-class, registro pós-aula, Estado pedagógico do ciclo, outras aulas e dados de outro papel permanecem intactos.
- [ ] O estado reiniciado persiste sem chaves residuais nem reaparecimento após recarga.

Adendo Normativo 03 · Alumni by Better · 24/08/2026
